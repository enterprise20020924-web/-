...
const artifactTerms = [
  ...
  { label: '工程占位词', pattern: /模板|提示词|工程词|占位符|placeholder/iu },
  {
    label: '内部标签',
    pattern:
      /<\/?(?:thinking|content|role_result|worldview_result|stage_early|stage_middle|stage_close|stage_common)\b/iu,
  },
  {
    label: '内置任务标签',
...
function collectContentWarnings(label: string, content: string, warnings: string[]): void {
  if (!content.trim()) {
    addWarning(warnings, `${label}为空，已用保底内容继续写入，请人工校对。`);
    return;
  }
  ...
  if (hit) {
    addWarning(warnings, `${label}包含工程词或占位内容：${hit.term.label}（${hit.match}），已继续写入，请人工校对。`);
  }
}
