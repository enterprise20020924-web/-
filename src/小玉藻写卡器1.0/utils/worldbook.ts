...
export async function ensureWorldbookBound(worldbookName: string, characterName: string): Promise<void> {
...
export async function clearWriterEntries(worldbookName: string): Promise<void> {
  ...
  await updateWorldbookWith(worldbookName, worldbook => worldbook.filter(entry => entry.extra?.source !== marker), {
    render: 'debounced',
  });
}
