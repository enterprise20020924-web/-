...
import { ensureCharacterDataCompatible } from './character';
...
export async function installMvuRegexes(characterName: string, statusHtml: string): Promise<void> {
  const targetCharacter = characterName.trim();
  if (!targetCharacter) throw new Error('角色卡名称不能为空');
  ...
  const mergeRegexes = (regexes: TavernRegex[]) => [
    ...regexes.filter(item => !scriptNames.has(item.script_name)),
    ...regexesToInstall,
  ];

  await ensureCharacterDataCompatible(targetCharacter);
  await updateCharacterWith(targetCharacter, character => {
    character.extensions ??= { regex_scripts: [], tavern_helper: { scripts: [], variables: {} } };
    character.extensions.regex_scripts = mergeRegexes(character.extensions.regex_scripts ?? []);
    return character;
  });

  if (getCurrentCharacterName() === targetCharacter) {
    await updateTavernRegexesWith(mergeRegexes, { scope: 'character' });
  }
}
