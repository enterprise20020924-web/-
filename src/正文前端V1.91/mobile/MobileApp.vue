    ...
    >
      ...
      <header class="bpm-topbar">
        ...
        <div class="bpm-topbar-actions">
          <div class="bpm-live-status">
            <span class="bpm-live-dot" aria-hidden="true"></span>
            <span>{{ context.during_streaming ? 'SYNC' : 'LIVE' }}</span>
          </div>
          <button
            type="button"
      ...
      <section class="bpm-stage" :aria-label="`${portraitSpeakerName}的剧情立绘`">
      ...
      <section class="bpm-dialogue-shell" aria-live="polite">
...
const dialogueSource = computed<DialogueSource>(() => ({
  ...
  userAliases: uniqueNonEmpty(['{{user}}', userAlias.value, runtime.SillyTavern?.name1, '你', '我']),
  primaryUserName: userAlias.value,
  secondaryUserNames: uniqueNonEmpty([runtime.SillyTavern?.name1]),
...
const activeSpeakerName = computed(() => {
  ...
  if (segment.kind === 'user') {
    return normalizeVisibleSpeaker(segment.speaker) ?? userAlias.value;
  }

  return normalizeVisibleSpeaker(segment.speaker) ?? portraitSpeakerName.value;
});
...
function isUserSpeaker(speaker: string) {
  ...
  return uniqueNonEmpty(['{{user}}', userAlias.value, runtime.SillyTavern?.name1, '你', '我']).some(
    ...
    fullbodyPortraitProfiles.find(profile =>
      profile.names.some(alias => {
        ...
        return (
          ...
          (normalizedAlias.length >= 2 && normalizedSpeaker.startsWith(normalizedAlias)) ||
          (normalizedSpeaker.length >= 2 && normalizedAlias.startsWith(normalizedSpeaker))
        );
      }),
    ) ?? null
  );
