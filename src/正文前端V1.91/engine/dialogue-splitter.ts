...
function splitQuotedParagraph(
  ...
  const quoteMatches = findQuoteMatches(paragraph).filter(
    match =>
      hasDialoguePunctuation(match.text) && !isSoundEffectQuote(match.text) && !isQuoteReferenceMatch(paragraph, match),
  );
  if (
    !forceSplitQuotes &&
    !quoteMatches.some(
...
function findContextualMentionedSpeaker(paragraph: string, source: DialogueSource) {
  const matches = createSpeakerCandidates(source)
    ...
    .filter(
      match =>
        CONTEXTUAL_MENTION_BEFORE_PATTERN.test(match.before) || CONTEXTUAL_MENTION_AFTER_PATTERN.test(match.after),
    )
    .sort(
      (left, right) =>
        left.range.index - right.range.index ||
...
function resolveMappedSegmentRole(displaySpeaker: string | null, source: DialogueSource) {
  const kind = displaySpeaker === null ? 'narration' : isSourceUserSpeaker(displaySpeaker, source) ? 'user' : 'npc';
  return {
    kind,
