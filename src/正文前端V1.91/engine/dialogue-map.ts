...
function sanitizeDialogueMapEntries(value: unknown): DialogueMapEntry[] {
  ...
  for (const rawEntry of rawEntries) {
    ...
    const anchor = sanitizeAnchor(entryRecord.anchor);
    if (!Number.isInteger(index) || index <= 0 || anchor.length === 0) {
      continue;
    }

    const lineStart = sanitizeLineNumber(entryRecord.line_start);
    const lineEnd = sanitizeLineNumber(entryRecord.line_end);
    const normalizedLineStart = lineStart !== null && lineEnd !== null && lineStart > lineEnd ? lineEnd : lineStart;
    const normalizedLineEnd = lineStart !== null && lineEnd !== null && lineEnd < lineStart ? lineStart : lineEnd;
    ...
    entries.push({
      ...
      anchor,
      speaker: sanitizeSpeakerLabel(entryRecord.speaker),
      focus: sanitizeSpeakerLabel(entryRecord.focus),
