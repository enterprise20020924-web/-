...
export function useJsonPatchAudit(
  context: Pick<ContentRendererContext, 'message_id' | 'set_variable_refresh_needed'>,
  ...
  function applyJsonPatchAuditOperation(
    ...
    return {
      ...
      item: createJsonPatchAuditItem(
        ...
        status,
        canonicalPath,
        `${pathMessage} ${actionMessage}${diffNote}`,
      ),
    };
  }
  ...
  function isWritableMvuPathSegments(segments: string[], valueSegments: string[], append: boolean) {
    ...
    if (root === '关系系统') {
      return (
        second !== undefined &&
        (second === '在场人物' || valueSegments.length === 2 || RELATIONSHIP_FIELD_NAMES.includes(third))
      );
    }
