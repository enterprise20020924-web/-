...
async function writeContentAlreadyExists(operation: WorkshopWriteOperation, normalizedPath: string): Promise<boolean> {
  if (!operation.skipIfContentIncludes) return false;
  return hasCharacterScriptContent(normalizedPath, operation.skipIfContentIncludes);
}
  ...
  for (const operation of operations) {
    ...
    if (operation.tool === 'Write') {
      ...
      const contentExists = await writeContentAlreadyExists(operation, normalized).catch(error => {
        records.push(makeRecord(operation, 'blocked', '脚本查重失败', errorMessage(error)));
        return undefined;
      });
      if (contentExists === undefined) continue;
      if (contentExists) {
        records.push(makeRecord(operation, 'skipped', '脚本本体已存在', '当前角色脚本库里已有同一 MVU 本体 import，将跳过创建。'));
        continue;
      }
      const node = await resolveWritableFileNode(normalized).catch(error => {
        records.push(makeRecord(operation, 'blocked', '预检失败', errorMessage(error)));
        return undefined;
      });
  ...
  for (const operation of operations) {
    try {
      ...
      if (operation.tool === 'Write') {
        ...
        const normalized = normalizeVirtualPath(operation.targetPath);
        if (normalized && await writeContentAlreadyExists(operation, normalized)) {
          records.push(makeRecord(operation, 'skipped', '已跳过脚本本体', '当前角色脚本库里已有同一 MVU 本体 import。'));
          continue;
        }
