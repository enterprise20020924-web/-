...
function findKnownSpeaker(label: string, source: DialogueSource) {
  ...
  const exactUserAlias = source.userAliases.find(alias => isSameSpeakerName(alias, normalizedLabel));
  if (exactUserAlias !== undefined) {
    return exactUserAlias;
  }
...
function splitQuotedParagraph(
  ...
  if (!quoteMatches.some(match => inferQuoteSpeaker(paragraph, match, source, nextParagraphs, previousSpeaker) !== null)) {
    return [
      withEmotion({
...
function splitParagraph(
  ...
  if (findQuoteMatches(paragraph).length > 0) {
    return splitQuotedParagraph(paragraph, paragraphIndex, source, nextParagraphs, previousSpeaker);
  }
...
function getDialogueMapCandidatePriority(segment: DialogueSegment, entry: DialogueMapEntry, anchorIndex: number) {
  ...
  const kindPriority =
    directSpeech && entry.kind === 'speech'
      ...
      : !directSpeech && entry.kind !== 'speech'
        ? 0
        : 1;
  return [kindPriority, anchorIndex, entry.i] as const;
}
...
function findDialogueMapEntryForSegment(
  ...
  if (normalizedSegmentText.length > 0) {
    const anchorMatches = dialogueMap
      .map((entry, entryIndex) => {
      if (usedEntries.has(entryIndex)) {
          return null;
      }
      ...
      const normalizedAnchor = normalizeMapAnchorText(entry.anchor);
        ...
        return {
          ...
          entryIndex,
          anchorIndex,
          priority: getDialogueMapCandidatePriority(segment, entry, anchorIndex),
        };
      })
      .filter((match): match is NonNullable<typeof match> => match !== null);

    if (anchorMatches.length > 0) {
      anchorMatches.sort((left, right) => compareDialogueMapCandidatePriority(left.priority, right.priority));
      const bestMatch = anchorMatches[0];
      return {
        entry: bestMatch.entry,
        entryIndex: bestMatch.entryIndex,
        consumedEntryIndexes: anchorMatches.map(match => match.entryIndex),
      };
    }
  }
...
function applyDialogueMapToSegments(segments: DialogueSegment[], source: DialogueSource) {
  ...
  const allowIndexFallback = dialogueMap.length === segments.length;
  return segments.map((segment, segmentIndex) => {
    const mapMatch = findDialogueMapEntryForSegment(segment, segmentIndex, dialogueMap, usedEntries, allowIndexFallback);
    if (mapMatch === null) {
      return {
        ...segment,
        focusSpeaker: segment.focusSpeaker ?? null,
    ...
    const shouldForceNarration = mapEntry.kind === 'speech' && !isDirectSpeechMapSegment(segment);
    const speaker = shouldForceNarration ? null : resolveMapDisplaySpeaker(mapEntry.speaker, source);
    const focusSpeaker = resolveMapKnownSpeaker(mapEntry.focus, source);
    const resolvedKind =
      speaker === null ? 'narration' : isSourceUserSpeaker(speaker, source) ? 'user' : 'npc';

    return withEmotion(
      {
        ...segment,
        kind: resolvedKind,
        side: resolvedKind === 'user' ? 'left' : resolvedKind === 'npc' ? 'right' : 'center',
        speaker,
        focusSpeaker,
        speakerSource: 'map',
...
export function splitDialogueSource(source: DialogueSource): DialogueSplitResult {
  ...
  const resolvedSegments = resolveSegmentsWithEvidence(segments, paragraphs, source);
  const displaySegments = mergeShortNarrationSegments(resolvedSegments);
  const mappedSegments = applyDialogueMapToSegments(displaySegments, source);
