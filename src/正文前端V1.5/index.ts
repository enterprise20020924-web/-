...
const originalContentPreserveTagPattern = /<\s*(?:option|choice|parallel)\b|&lt;\s*(?:option|choice|parallel)\b/i;
...
function findDisplayedContentRange($mesText: JQuery<HTMLElement>, content: string): Range | null {
  ...
  return findDisplayedContentRangeInRoot(root, content);
}

function findDisplayedContentRangeInRoot(root: HTMLElement, content: string): Range | null {
...
function isPreservedOriginalContentElement(element: Element) {
  ...
  if (['option', 'choice', 'parallel', 'select', 'button', 'input', 'textarea', 'a'].includes(tagName)) {
    return true;
  }
  ...
  return (
    /\b(option|options|choice|choices|parallel|parallels|select|button|btn)\b/.test(markerText) ||
...
function findOriginalContentPreserveBoundary(root: HTMLElement): RangeBoundaryPoint | null {
  ...
  while (current !== null) {
    if (current.nodeType === Node.TEXT_NODE) {
      ...
      const preservedTagIndex = text.search(originalContentPreserveTagPattern);
      if (preservedTagIndex >= 0) {
        return { node: current as Text, offset: preservedTagIndex };
      }
...
function hasOriginalContentPreserveTail(message: string) {
  const withoutDialogueMap = stripDialogueMapBlocks(message);
  const closingContentTagMatch = withoutDialogueMap.match(/<\/(?:content|maintext)\s*>/i);
  const tail =
    closingContentTagMatch === null || closingContentTagMatch.index === undefined
      ? withoutDialogueMap
      : withoutDialogueMap.slice(closingContentTagMatch.index + closingContentTagMatch[0].length);

  return originalContentPreserveTagPattern.test(tail);
}

function createOriginalContentHideRangeToDisplayedContentEnd(root: HTMLElement, content: string) {
  const contentRange = findDisplayedContentRangeInRoot(root, content);
...
function createOriginalContentHideRange(root: HTMLElement, content: string, sourceHasPreservedTail: boolean) {
  ...
  if (preserveBoundary === null) {
    if (sourceHasPreservedTail) {
      return createOriginalContentHideRangeToDisplayedContentEnd(root, content);
    }
...
function hideDisplayedContentForRenderer(messageId: number, mesText: HTMLElement, content: string, sourceMessage: string) {
  return runWithSuppressedMessageMutation(mesText, () => {
    restoreHiddenContent(messageId, mesText);

    const range = createOriginalContentHideRange(mesText, content, hasOriginalContentPreserveTail(sourceMessage));
    if (range === null) {
      return false;
    }
...
function renderOneMessage(messageId: number, streamMessage?: string) {
  ...
  if (existingState !== undefined) {
    if (isNodeConnectedToItsDocument(existingState.host) && $messageElement[0]?.contains(existingState.host)) {
      ...
      if (shouldRefreshHiddenContent && !hideDisplayedContentForRenderer(messageId, mesText, content, message)) {
  ...
  if (!hideDisplayedContentForRenderer(messageId, mesText, content, message)) {
    $host.remove();
    console.warn(`[content-chat-renderer] content range not found in message ${messageId}`);
    scheduleRangeRetry(messageId);
    return;
