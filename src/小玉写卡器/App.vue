  ...
  <main class="writer-shell">
    ...
    <section v-if="step === 'setup'" class="setup-step">
      <div class="setup-card">
        ...
        <div class="summary persona-entry">
          <strong>单独生成人设</strong>
          <span>不绑定世界书，不写入角色卡；只输入人设信息，输出普通调色盘或多阶段调色盘。</span>
        </div>

        <button class="secondary wide" :disabled="isBusy" @click="openPersonaGenerator">只生成人设</button>
      </div>
    </section>

    <section v-else-if="step === 'persona'" class="workspace persona-workspace">
      <section class="editor-panel persona-editor">
        <div class="section-head">
          <div>
            <h2>单独生成人设</h2>
            <p>只调用人设/调色盘知识，不写入世界书。</p>
          </div>
        </div>
...
如果选择多阶段调色盘，可以补充初识期、熟悉期、亲近期分别会怎样变化。"
        ...
        <div class="persona-actions">
          <button class="ghost" :disabled="isBusy" @click="step = 'setup'">返回设置</button>
          <button class="primary" :disabled="isBusy || !canRunPersona" @click="runPersonaOnly">
      ...
      <aside class="result-panel">
        <div class="section-head">
          <h2>人设结果</h2>
          <button class="ghost" :disabled="isBusy" @click="clearPersonaResult">清空</button>
        </div>
        ...
        <div v-if="personaResult" class="preview persona-result">
          <strong>{{ personaModeName(personaResult.mode) }}</strong>
...
import { generatePersonaOnly, generateRole, generateWorldview } from './utils/generation';
...
type WriterStep = 'setup' | 'editor' | 'persona';
...
function noteGenerationWarnings(source: string, warnings: string[]) {
  ...
  toastr.warning(`${source}存在标签或内容提醒，已继续生成`, '玉藻前写卡器');
}
    ...
    ...activeRoles.flatMap((role, index) => [
      {
        ...
        title: `生成${role.label}`,
        detail: '调用调色盘浓缩知识扩写角色',
        status: 'pending' as WorkStatus,
      },
      {
        key: `write-role-${index}`,
        title: `写入${role.label}`,
        detail: '写入角色速览、基础、调色盘、二次解释、EJS',
        status: 'pending' as WorkStatus,
      },
    ]),
...
async function runPipeline(startIndex = 0) {
  ...
  if (startIndex === 0) {
    previewText.value = '';
    generationWarnings.value = [];
    const activeRoles = roles.filter(role => role.seed.trim()).map(role => ({ ...role }));
...
.warning-panel {
  ...
  gap: 7px;
  margin-top: 12px;
  border: 1px solid rgba(154, 106, 25, 0.24);
  border-radius: 8px;
  background: rgba(154, 106, 25, 0.08);
  padding: 11px;
  color: #76510f;
  font-size: 13px;
  line-height: 1.45;
...
@media (max-width: 980px) {
  ...
  .result-panel {
    ...
    overflow: visible;
    padding: 14px calc(14px + env(safe-area-inset-right, 0px)) 14px calc(14px + env(safe-area-inset-left, 0px));
  }
