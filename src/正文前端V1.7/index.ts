...
function parseChoiceOptions(content: string): ChoiceOption[] {
  ...
  for (const rawLine of cleanedContent.split('\n')) {
    ...
    const bracketMatch = line.match(/^\[?\s*([A-Da-d1-4])\s*(?:[.。:：、)|]|[-–—]\s*)\s*(.+?)\s*\]?$/);
    const plainMatch = line.match(/^(?:[-*]\s*)?([A-Da-d1-4])\s*[.。:：、)]\s*(.+)$/);
  ...
  return options.filter(option => option.text.length > 0).slice(0, 4);
}
...
function prepareRendererMessage(messageId: number, message: string, chatMessage: ChatMessage | undefined) {
  ...
  if (shouldCacheExtractedMap) {
    ...
    if (typeof setMessages === 'function') {
      void setMessages([{ message_id: messageId, message, data: nextData }], { refresh: 'none' }).catch(error => {
