...
function hasPreservedOriginalContentDescendant(element: Element) {
  return Array.from(element.querySelectorAll('*')).some(isPreservedOriginalContentElement);
}

function isPreservedOriginalContentNode(node: Node) {
  if (node.nodeType !== Node.ELEMENT_NODE) {
    return false;
  }

  const element = node as Element;
  return isPreservedOriginalContentElement(element);
}

function wrapHiddenOriginalContentNode(messageId: number, referenceNode: Node, nodeToHide: Node) {
  const existingPreviousWrapper =
    referenceNode.previousSibling instanceof HTMLElement &&
    referenceNode.previousSibling.matches(getHiddenContentSelector(messageId))
      ? referenceNode.previousSibling
      : null;
  const hiddenWrapper = existingPreviousWrapper ?? referenceNode.ownerDocument.createElement('span');

  if (existingPreviousWrapper === null) {
    hiddenWrapper.dataset.baipoContentRendererHidden = String(messageId);
    hiddenWrapper.style.display = 'none';
    referenceNode.parentNode?.insertBefore(hiddenWrapper, referenceNode);
  }

  hiddenWrapper.append(nodeToHide);
}

function hideOriginalContentNodesExceptPreserved(messageId: number, root: HTMLElement) {
  let hiddenCount = 0;

  for (const node of Array.from(root.childNodes)) {
    if (isPreservedOriginalContentNode(node)) {
      continue;
    }

    if (node.nodeType === Node.ELEMENT_NODE && hasPreservedOriginalContentDescendant(node as Element)) {
      if (hideOriginalContentNodesExceptPreserved(messageId, node as HTMLElement)) {
        hiddenCount += 1;
      }
      continue;
    }

    wrapHiddenOriginalContentNode(messageId, node, node);
    hiddenCount += 1;
  }
...
function hideDisplayedContentForRenderer(messageId: number, mesText: HTMLElement, content: string) {
  return runWithSuppressedMessageMutation(mesText, () => {
    ...
    if (mesText.childNodes.length === 0) {
      return false;
    }

    return hideOriginalContentNodesExceptPreserved(messageId, mesText);
  });
}
