...
function capturePlanProtocolFromLatestMessage() {
  try {
    ...
    const { draft, update } = parsePlanProtocolFromText(latest.message);
    if (draft) {
      planStore.setPendingDraft(draft);
      toastr.info('已识别 PlanDraft，请在计划面板确认');
    }
    if (update) {
      planStore.setPendingUpdate(update);
      toastr.info('已识别 PlanUpdate，请在计划面板确认');
    }
  } catch (error) {
