...
export const useWorkshopStore = defineStore('workshop', () => {
  ...
  function buildPlanDraftPrompt(currentCharacterName?: string | null): string {
    const task = selectedTask.value;
    const targetName = draft.targetName.trim() || currentCharacterName || '<请先确认目标角色或世界书>';
    const userGoal = draft.userNotes.trim() || draft.assistantNotes?.trim() || '用户尚未补充详细目标，请基于当前任务先起草可执行计划，并在 questions 中列出必须追问的问题。';

    return [
      '进入【明月秋青】标准协作模式的计划草拟流程。',
      '',
      `当前任务：${task.title}`,
      `任务ID：${task.id}`,
      `目标：${targetName}`,
      '',
      '请根据用户目标起草一个可执行计划。只输出一个 JSON 代码块，不要写额外正文。',
      'JSON schema 固定为 mingyue.plan_draft.v1。',
      '',
      '计划约束：',
      '- todo 数量控制在 3-7 个。',
      '- 每个 todo 要能被用户确认完成。',
      '- 不要把写入、归档、关闭计划设为 AI 自动执行。',
      '- 所有写入必须等待前端写入计划确认。',
      '- 如果信息不足，confidence 使用 needs_input，并在 questions 中列出问题。',
      '',
      '任务要求：',
      `- ${task.summary}`,
      `- 第一步：${task.firstAction}`,
