  ...
  <div class="tamamo-opening">
    ...
    <header class="hero-header">
      ...
      <div class="hero-copy">
        ...
        <h1>千面映一身</h1>
        <p class="hero-sub">亲爱的，可以好好回忆下自己是什么样的人再做出选择哦♥~Mikon</p>
      </div>
    ...
    <section class="form-scroll">
      ...
      <div class="form-body">
        ...
        <div class="field-group">
          <label class="field-label"> 姓名 <span class="required-star">*</span> </label>
          <input v-model="formData.name" type="text" class="tamamo-input" placeholder="例如：雨宫 夜澪" />
        </div>

        <!-- 性别 -->
        <div class="field-group">
          <label class="field-label"> 性别 <span class="required-star">*</span> </label>
        ...
        <!-- 身材外貌 -->
        <div class="field-group">
          <label class="field-label"> 身材外貌 <span class="required-star">*</span> </label>
          <textarea
            v-model="formData.identity"
            class="tamamo-input tamamo-textarea"
            placeholder="例如：170cm｜长发及腰｜丰满身材｜冷艳面容｜蜜桃肌肤"
          ></textarea>
        </div>

        <!-- 时间线 -->
        <div class="field-group">
          <label class="field-label field-label-hint">时间线</label>
          <div class="timeline-nodes">
            <div
              ...
              class="timeline-node"
              :class="{
                'timeline-node-disabled': isTagDisabled(index),
                'timeline-node-expanded': expandedNode === index,
                'timeline-node-selected': selectedNode === index,
              }"
            >
              <button class="timeline-node-btn" :disabled="isTagDisabled(index)" @click="toggleNode(index)">
                <span class="timeline-node-indicator">{{ selectedNode === index ? '◆' : '◇' }}</span>
                <span class="timeline-node-label">{{ node.name }}</span>
                <span class="timeline-node-arrow">{{ expandedNode === index ? '▲' : '▼' }}</span>
              </button>
              <Transition name="accordion">
                <div v-if="expandedNode === index" class="timeline-node-desc">
                  <p>{{ node.desc }}</p>
                </div>
              </Transition>
            </div>
    ...
    <footer class="action-bar">
      ...
      <button class="action-btn action-btn-secondary" @click="handleCopy">复制设定文本</button>
      <button class="action-btn action-btn-secondary" @click="handleClear">清空表单</button>
...
const timelineNodes = [
  {
    name: '时间节点1',
    desc: '希安还未正式转入{{user}}学校，{{user}}能防患于未然吗？还是输到一无所有连自己也赔进去呢？',
  },
  {
    name: '时间节点2',
    desc: '女友响和青梅竹马朋香已经委身于希安并开始调教{{user}}，{{user}}还能逆转局势吗？还是跟着两人一块沉沦呢？',
  },
  { name: '时间节点3', desc: '希安虽然没有对其他人下手，但{{user}}已经接近堕落边缘了。是彻底沦陷还是绝地反击？' },
  { name: '时间节点4', desc: '迦南登门拜访天王寺一家，她真如表面看上去那般和蔼可亲吗？' },
  {
    name: '时间节点5',
    desc: '女友响和青梅竹马朋香已经委身于希安并开始调教{{user}}，{{user}}还能逆转局势吗？还是跟着两人一块沉沦呢？',
  },
...
function parsePagerText(text: string): { current: number; total: number } | null {
  const clean = text.replace(/\u200b/g, '').trim();
  const matched = clean.match(/^(\d+)\s*\/\s*(\d+)$/);
...
async function tryJumpToOpeningSceneByTimeline(): Promise<boolean> {
  if (selectedNode.value === null) return false;

  // 需求：时间节点1 -> 第2个开头，时间节点2 -> 第3个开头
  const targetOpening = selectedNode.value + 2;

  // 优先使用酒馆助手接口切换开场（稳定，不依赖隐藏/可见按钮）
  try {
    const firstMessage = getChatMessages(0, { include_swipes: true })[0] as ChatMessageSwiped | undefined;
    if (firstMessage?.swipes?.length) {
      const targetSwipeId = targetOpening - 1; // 开场1对应 swipe_id=0
      if (targetSwipeId >= 0 && targetSwipeId < firstMessage.swipes.length) {
        await setChatMessages([{ message_id: firstMessage.message_id, swipe_id: targetSwipeId }], {
          refresh: 'affected',
        });
        console.info(`[开场表格] 已通过 setChatMessages 切换到第${targetOpening}个开头`);
        return true;
      }
      console.warn(`[开场表格] 目标开头超出范围：target=${targetOpening}, total=${firstMessage.swipes.length}`);
      return false;
    }
  } catch (error) {
    console.warn('[开场表格] 接口切换开场失败，回退到 DOM 点击方案', error);
  }
  ...
  const isVisible = (el: HTMLElement) => {
    ...
    const style = window.parent?.getComputedStyle(el) ?? getComputedStyle(el);
    return rect.width > 0 && rect.height > 0 && style.display !== 'none' && style.visibility !== 'hidden';
  };

  const candidates = Array.from(parentDoc.querySelectorAll<HTMLElement>('span, div, p, strong'));
  const pagers = candidates
    .map(node => ({ node, pager: parsePagerText(node.textContent ?? '') }))
    .filter((item): item is { node: HTMLElement; pager: { current: number; total: number } } => item.pager !== null)
    .sort((a, b) => b.pager.total - a.pager.total);

  for (const { node, pager } of pagers) {
    if (targetOpening > pager.total) continue;

    const container = node.closest<HTMLElement>('.mes, .swipeRightBlock, section, article, div') ?? node.parentElement;
    if (!container) continue;

    const searchRoots = [container, container.parentElement, container.parentElement?.parentElement].filter(
      Boolean,
    ) as HTMLElement[];

    const buttons = searchRoots.flatMap(root =>
      Array.from(
        root.querySelectorAll<HTMLElement>(
          'button, [role="button"], .swipe_right, .swipe_left, .fa-chevron-right, .fa-chevron-left, .interactable',
        ),
      ),
    );

    const uniqButtons = Array.from(new Set(buttons));

    const nextBtn =
      uniqButtons.find(btn => {
        if (!isVisible(btn) || btn.hasAttribute('disabled')) return false;
        const text = (btn.textContent ?? '').trim();
        const cls = btn.className ?? '';
        const title = btn.getAttribute('title') ?? '';
        return /[▶▸►＞>⮞]/u.test(text) || /(swipe_right|chevron-right)/u.test(cls) || /(下一|next)/iu.test(title);
      }) ?? null;
    ...
    const prevBtn =
      uniqButtons.find(btn => {
        ...
        const cls = btn.className ?? '';
        const title = btn.getAttribute('title') ?? '';
        return /[◀◂◄＜<⮜]/u.test(text) || /(swipe_left|chevron-left)/u.test(cls) || /(上一|prev)/iu.test(title);
      }) ?? null;

    const delta = targetOpening - pager.current;
    if (delta === 0) {
      console.info(`[开场表格] 当前已是第${targetOpening}个开头，无需滑动`);
      return true;
    }

    const clickTarget = delta > 0 ? nextBtn : prevBtn;
    ...
    console.info(`[开场表格] 已通过 DOM 点击切换到第${targetOpening}个开头`);
    return true;
  }
...
function toggleNode(index: number) {
  ...
  if (selectedNode.value === index) {
    ...
    expandedNode.value = index;
  }
}

function isTagDisabled(index: number): boolean {
  const gender = formData.value.gender;
  if (!gender) return true;
  if (gender === '男' && index <= 2) return true;
  if (gender === '女' && index >= 3) return true;
  return false;
}
...
function buildSettingText(): string {
  const lines: string[] = [];
  lines.push(`姓名：${formData.value.name.trim()}`);
  lines.push(`性别：${formData.value.gender}`);
  lines.push(`外貌：${formData.value.identity.trim()}`);
  if (selectedNode.value !== null) {
    lines.push(`时间线：${timelineNodes[selectedNode.value].name}`);
  }
  const extra = formData.value.extra.trim();
  if (extra) {
    lines.push(`额外设定：${extra}`);
  }
  return lines.join('\n');
}

function buildUserEntryContent(): string {
  const lines: string[] = [];
  lines.push('<用户信息>');
  lines.push('【角色档案】');
  lines.push(`姓名：${formData.value.name.trim()}`);
...
function getWorldbookCandidates(): string[] {
  ...
  pushName(getChatWorldbookName('current'));

  const charBooks = getCharWorldbookNames('current');
  pushName(charBooks.primary);
  charBooks.additional.forEach(pushName);

  getGlobalWorldbookNames().forEach(pushName);

  return [...set];
}

async function writeUserEntryToWorldbook(content: string): Promise<boolean> {
  const worldbookNames = getWorldbookCandidates();

  for (const worldbookName of worldbookNames) {
    try {
      const worldbook = await getWorldbook(worldbookName);
      const userEntry = worldbook.find(entry => entry.name.trim().toLowerCase() === 'user');
      if (!userEntry) continue;

      await updateWorldbookWith(worldbookName, entries =>
        entries.map(entry => {
          if (entry.uid !== userEntry.uid) return entry;

          const current = String(entry.content ?? '');
          const replaced =
            current.includes('<用户信息>') && current.includes('</用户信息>')
              ? current.replace(/<用户信息>[\s\S]*?<\/用户信息>/u, content)
              : content;
      ...
      console.info(`[开场表格] 已写入 user 条目: ${worldbookName}`);
      return true;
    } catch (error) {
      console.warn(`[开场表格] 读取/写入世界书失败: ${worldbookName}`, error);
    }
  }
...
async function handleStart() {
  ...
  try {
    ...
    const data = {
      ...
      timeline: selectedNode.value !== null ? timelineNodes[selectedNode.value].name : '',
    };
    replaceVariables(data, { type: 'chat' });

    const synced = await writeUserEntryToWorldbook(buildUserEntryContent());
    if (!synced) {
      console.warn('[开场表格] 未找到名为 user 的世界书条目');
      toastr.warning('已写入聊天变量，但未找到 user 世界书条目');
    }

    const jumped = await tryJumpToOpeningSceneByTimeline();

    statusText.value = jumped ? '灵基铭刻完成，已定位对应开头 ✦' : '灵基铭刻完成 ✦';
    toastr.success(jumped ? '开局身份已录入，并已滑动到对应开头！' : '开局身份已录入！');
    if (!jumped) {
      console.warn('[开场表格] 未找到开头滑动控件，已跳过自动滑动');
    }
  } catch (e) {
...
@keyframes sakuraFall {
  ...
  100% {
    transform: translateY(calc(100cqb + 40px)) rotate(360deg);
    opacity: 0;
  }
}
...
.foxfire-dot:nth-child(6) {
  left: 12%;
  top: 72%;
  width: 4px;
  height: 4px;
  animation-duration: 13s, 2.2s;
  animation-delay: -5s;
}
.foxfire-dot:nth-child(7) {
  left: 92%;
  top: 48%;
  width: 3px;
  height: 3px;
...
.tamamo-moon {
  ...
  box-shadow:
    0 0 16px rgba($gold, 0.4),
    0 0 40px rgba($gold, 0.15);
...
@keyframes moonGlow {
  ...
  100% {
    box-shadow:
      0 0 24px rgba($gold, 0.6),
      0 0 60px rgba($gold, 0.25);
  }
}
...
@keyframes bannerPulse {
  ...
  100% {
    box-shadow: 0 0 28px rgba($gold, 0.22);
  }
}
...
.scroll-rod {
  ...
  background:
    repeating-linear-gradient(
      90deg,
      rgba(90, 60, 20, 0.15) 0px,
      transparent 2px,
      transparent 8px,
      rgba(90, 60, 20, 0.1) 10px
    ),
    repeating-linear-gradient(85deg, rgba(60, 40, 10, 0.08) 0px, transparent 1px, transparent 14px),
    linear-gradient(
      180deg,
      #5a3c14 0%,
      #8b6420 10%,
...
.form-body {
  padding: 20px 18px;
  background:
    repeating-linear-gradient(90deg, rgba(80, 55, 20, 0.04) 0px, transparent 1px, transparent 12px),
    repeating-linear-gradient(87deg, rgba(60, 40, 15, 0.03) 0px, transparent 1px, transparent 18px),
...
@keyframes bodyFoxfire {
  0%,
  100% {
    border-left-color: rgba($gold, 0.2);
    border-right-color: rgba($gold, 0.2);
  }
  33% {
    border-left-color: rgba(74, 140, 255, 0.2);
    border-right-color: rgba(74, 140, 255, 0.15);
  }
  66% {
    border-left-color: rgba(232, 160, 191, 0.18);
    border-right-color: rgba(232, 160, 191, 0.15);
  }
}
...
.form-scroll::before {
  ...
  box-shadow:
    -1px -1px 4px rgba($gold, 0.15),
    inset 1px 1px 2px rgba($gold, 0.1);
}
...
.example-tag-disabled:hover {
  ...
  color: rgba(200, 190, 200, 0.4);
  box-shadow: none;
  transform: none;
}

/* =====================================================
   TIMELINE NODES (Accordion)
   ===================================================== */
.timeline-nodes {
  display: grid;
  gap: 10px;
...
.timeline-node-selected {
  ...
  box-shadow:
    0 0 20px rgba($gold, 0.12),
    inset 0 0 0 1px rgba($gold, 0.08);
}

.timeline-node-selected .timeline-node-btn {
  color: $gold-light;
}
...
.timeline-node-selected .timeline-node-indicator {
  color: $gold-light;
  text-shadow: 0 0 8px rgba($gold, 0.5);
}
...
.timeline-node-btn:hover:not(:disabled) {
  background: rgba($gold, 0.06);
  color: $gold-light;
}

.timeline-node-btn:disabled {
  opacity: 0.35;
  cursor: not-allowed;
  color: rgba(200, 190, 200, 0.4);
}
...
.timeline-node-desc p {
  margin: 0;
  padding: 12px 14px;
  font-size: 12.5px;
  line-height: 1.75;
  color: rgba($gold-pale, 0.78);
  background: rgba($purple-deep, 0.5);
  border-radius: 8px;
