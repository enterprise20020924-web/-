...
const PRONOUN_ACTION_LABEL_PATTERN =
  /^(?:她|他|它|TA|Ta|ta|我|你).*(?:心里|内心|心中|脑海|默念|心想|想道|想着|低声|轻声|柔声|冷声|厉声|开口|回应|说|问|喊|道|念叨|嘟囔|低语|喃喃)$/;
...
const LEADING_ACTION_AFTER_SPEAKER_PATTERN =
  /^(?:从|向|朝|对|把|被|在|低头|抬头|转身|回头|走|跑|站|坐|靠|停|凑|退|伸|收|拿|放|推|拉|扶|抱|夹|拍|点|摇|眨|睁|眯|皱|抿|咬|捂|笑|轻笑|微笑|苦笑|愣|怔|叹|开口|回应|说|问|喊|道)/;
const LEADING_ACTION_SUBJECT_PATTERN =
  /^([\u4e00-\u9fa5A-Za-z]{2,10})(?=(?:从|向|朝|对|把|被|在|低头|抬头|转身|回头|走|跑|站|坐|靠|停|凑|退|伸|收|拿|放|推|拉|扶|抱|夹|拍|点|摇|眨|睁|眯|皱|抿|咬|捂|笑|轻笑|微笑|苦笑|愣|怔|叹|开口|回应|说|问|喊|道))/;
...
function inferEmbeddedActionSpeaker(paragraph: string, source: DialogueSource) {
  const matches = createSpeakerCandidates(source)
    ...
    .map(speaker => {
      const index = paragraph.indexOf(speaker);
      return { speaker, index };
    })
    .filter(match => match.index >= 0)
    .map(match => ({
      ...match,
      prefix: paragraph.slice(0, match.index).trim(),
      rest: paragraph.slice(match.index + match.speaker.length).trimStart(),
    }))
...
function resolveColonSpeaker(label: string, source: DialogueSource, previousSpeaker: string | null) {
  ...
  if (previousSpeaker !== null && PRONOUN_ACTION_LABEL_PATTERN.test(normalizedLabel)) {
...
export function splitDialogueSource(source: DialogueSource): DialogueSplitResult {
  ...
  const segments: DialogueSegment[] = [];
  let recentSpeaker: string | null = null;
  let recentSpeakerDistance = Number.POSITIVE_INFINITY;

  paragraphs.forEach((paragraph, paragraphIndex) => {
    const carriedSpeaker =
      recentSpeakerDistance <= SPEAKER_CARRY_MAX_PARAGRAPHS ? recentSpeaker : null;
    ...
    if (explicitSpeaker !== null) {
      recentSpeaker = explicitSpeaker;
      recentSpeakerDistance = 0;
      return;
    }

    recentSpeakerDistance += 1;
  });
