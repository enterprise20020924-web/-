...
const FOCUS_MENTION_AFTER_PATTERN =
  /^\s*(?:，|,)?\s*(?:的[^。！？!?；;\n]{0,24})?(?:(?:已经|刚刚|刚|立刻|随即|猛地|突然|悄悄|几乎|快要)[^。！？!?；;\n]{0,12})?(?:正|正在|仍然|仍旧|依旧|始终|一直|只是|低头|抬手|抬头|抬起头|转头|转过头|转身|回头|看向|垂眸|坐|站|靠|走|走来|走过来|走进|走进来|跑|停|拿|拿着|拿起|捧|捧着|抱|抱着|夹|夹着|翻|把|向|朝|对|在|坐在|站在|靠在|位于|端|端着|喝|喝了|写|写下|合上|停|停了|落|落回|移|移开|清点|核对|打勾|打着勾|触碰|碰到|碰|摸|伸向|伸出|抓住|攥住|捏住|抽出|放下|收回)/;
...
function hasDialogueLikeQuote(text: string) {
  return findQuoteMatches(text).some(match => hasDialoguePunctuation(match.text) && !isQuoteReferenceMatch(text, match));
}
...
function splitQuotedParagraph(
  ...
  const quoteMatches = findQuoteMatches(paragraph).filter(
    match => hasDialoguePunctuation(match.text) && !isQuoteReferenceMatch(paragraph, match),
  );
...
function addSpeakerEvidence(
  ...
  const speaker = findKnownSpeaker(rawSpeaker, source) ?? rawSpeaker.trim();
  if (!isPlausibleSpeakerLabel(speaker)) {
    return;
  }
...
function resolveSegmentsWithEvidence(
  ...
  return segments.map(segment => {
    if (!isQuoteDialogueSegment(segment)) {
      if (
        segment.speaker !== null &&
        (segment.id.includes('-colon-') || segment.id.includes('-quote-')) &&
        isStableContextSpeaker(segment.speaker, source)
      ) {
    ...
    if (speaker !== null && isStableContextSpeaker(speaker, source)) {
...
export function splitDialogueSource(source: DialogueSource): DialogueSplitResult {
  ...
  return {
    segments: mergeShortNarrationSegments(resolvedSegments),
