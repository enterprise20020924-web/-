<template>
  <div class="abyss-root" @click="handleBackdropReturnHome">
    <div class="phone-shell" @click.stop>
      ...
      <div class="phone-screen">
        ...
        <main class="page-content">
          <section v-if="activeTab === 'forum'" class="tab-page">
            ...
            <article class="panel-card interaction-overview">
              ...
              <div class="overview-actions">
                ...
                <button class="action-btn" @click="activeTab = 'hypnosis'">言灵</button>
          ...
          <section v-else-if="activeTab === 'map'" class="tab-page map-page">
            <article class="map-container">
              <div class="map-overlay">
                <div class="map-header" :class="{ 'is-collapsed': isMapHeaderCollapsed }">
                  <div class="map-header-actions">
                    <button class="map-home-btn" @click.stop="returnToHomeFromMap">← 返回主页</button>
                  </div>
                  <div class="map-search-bar" @click="isMapHeaderCollapsed = !isMapHeaderCollapsed" style="cursor: pointer;">
                    <span class="icon">🔍</span>
                    <input type="text" placeholder="搜索地点或建筑..." style="pointer-events: none;" />
                    <span class="icon-mic" @click.stop="isMapHeaderCollapsed = false">🎤</span>
                  </div>
                  <div class="map-directions-bar" v-show="!isMapHeaderCollapsed">
                    <div class="route-info">
                      <span class="dot origin"></span><span class="route-text">天王寺家 (当前位置)</span>
                    </div>
                    <div class="route-line-vertical"></div>
                    <div class="route-info">
                      <span class="dot dest"></span><span class="route-text">{{ routeDestName }} (目的地)</span>
                    </div>
                    <div class="route-transport">
                      <span
                        v-for="transport in routeTransportModes"
                        :key="`${routeDestName}-${transport.icon}-${transport.time}`"
                        class="transport"
                        :class="{ active: transport.active }"
                      >
                ...
                <div 
                  class="map-view"
                  ref="mapViewRef"
                  @mousedown.prevent="startMapDrag"
                  @mousemove.prevent="onMapDrag"
                  @mouseup="endMapDrag"
                  @mouseleave="endMapDrag"
                  @touchstart="startMapDrag"
                  @touchmove.prevent="onMapDrag"
                  @touchend="endMapDrag"
                  @wheel.prevent="onMapWheel"
                >
                  <div class="map-transform-layer" :style="{ transform: `translate(${mapOffsetX}px, ${mapOffsetY}px) scale(${mapScale})`, transformOrigin: 'top left', width: '100%', height: '100%', position: 'absolute' }">
                    <div class="map-bg-grid"></div>

                    <div class="map-buildings">
                      <div class="building-3d" style="top: 15%; left: 20%; width: 40px; height: 60px;"></div>
                      <div class="building-3d tall" style="top: 25%; left: 45%; width: 60px; height: 50px;"></div>
                      <div class="building-3d" style="top: 48%; left: 10%; width: 80px; height: 80px;"></div>
                      <div class="building-3d tall" style="top: 55%; left: 60%; width: 90px; height: 70px;"></div>
                      <div class="building-3d" style="top: 70%; left: 30%; width: 50px; height: 90px;"></div>
                      <div class="building-3d tall" style="top: 85%; left: 75%; width: 70px; height: 100px;"></div>
                      
                      <div class="building-3d" style="top: 35%; left: 70%; width: 50px; height: 40px;"></div>
                      <div class="building-3d tall" style="top: 60%; left: 20%; width: 40px; height: 50px;"></div>
                      <div class="building-3d" style="top: 80%; left: 50%; width: 60px; height: 60px;"></div>
                      <div class="building-3d tall" style="top: 40%; left: 85%; width: 50px; height: 70px;"></div>
                      <div class="building-3d" style="top: 10%; left: 80%; width: 70px; height: 50px;"></div>
                      <div class="building-3d tall" style="top: 90%; left: 15%; width: 80px; height: 40px;"></div>
                    </div>

                    <div class="map-zone residential-zone">
                      <div
                        class="map-pin pin-start"
                        :class="{ 'is-selected': isRouteSelected('天王寺家') }"
                        style="top: 20%; left: 30%"
                        @click.stop="navigateToPin(30, 20, '天王寺家')"
                      >
                        <div class="pin-marker"><span class="pin-inner"></span></div>
                        <span class="pin-label">天王寺家</span>
                      </div>
                      <div
                        class="map-pin pin-neighbor"
                        :class="{ 'is-selected': isRouteSelected('大神家') }"
                        style="top: 10%; left: 10%"
                        @click.stop="navigateToPin(10, 10, '大神家')"
                      >
                        <span class="pin-dot"></span>
                        <span class="pin-label">大神家</span>
                      </div>
                      <div
                        class="map-pin pin-neighbor"
                        :class="{ 'is-selected': isRouteSelected('迦南家') }"
                        style="top: 25%; left: 50%"
                        @click.stop="navigateToPin(50, 25, '迦南家')"
                      >
                        <span class="pin-dot"></span>
                        <span class="pin-label">迦南家</span>
                      </div>
                      <div
                        class="map-pin pin-further"
                        :class="{ 'is-selected': isRouteSelected('一条家') }"
                        style="top: 35%; left: 15%"
                        @click.stop="navigateToPin(15, 35, '一条家')"
                      >
                        <span class="pin-dot"></span>
                        <span class="pin-label">一条家</span>
                      </div>
                      <div class="zone-label" style="top: 40%; left: 30%;">住宅区</div>
                    </div>

                    <svg class="map-route-svg" viewBox="0 0 100 100" preserveAspectRatio="none">
                      <path class="route-path-bg" :d="routePathD" />
                      <path class="route-path" :d="routePathD" />
                    </svg>

                    <div class="map-zone commercial-zone">
                      <div class="zone-label" style="top: 50%; left: 65%;">主要街区</div>

                      <div
                        class="map-pin pin-poi pin-cafe"
                        :class="{ 'is-selected': isRouteSelected('黑桃咖啡厅') }"
                        style="top: 55%; left: 55%"
                        @click.stop="navigateToPin(55, 55, '黑桃咖啡厅')"
                      >
                        <span class="pin-icon">☕</span><span class="pin-label">黑桃咖啡厅</span>
                      </div>
                      <div
                        class="map-pin pin-poi pin-gym"
                        :class="{ 'is-selected': isRouteSelected('太阳女性健身房') }"
                        style="top: 60%; left: 75%"
                        @click.stop="navigateToPin(75, 60, '太阳女性健身房')"
                      >
                        <span class="pin-icon">💪</span><span class="pin-label">太阳女性健身房</span>
                      </div>
                      <div
                        class="map-pin pin-poi pin-church"
                        :class="{ 'is-selected': isRouteSelected('黑日教堂') }"
                        style="top: 70%; left: 60%"
                        @click.stop="navigateToPin(60, 70, '黑日教堂')"
                      >
                        <span class="pin-icon">⛪</span><span class="pin-label">黑日教堂</span>
                      </div>
                      <div
                        class="map-pin pin-poi pin-club"
                        :class="{ 'is-selected': isRouteSelected('黑桃夜总会') }"
                        style="top: 80%; left: 70%"
                        @click.stop="navigateToPin(70, 80, '黑桃夜总会')"
                      >
                        <span class="pin-icon">🍸</span><span class="pin-label">黑桃夜总会</span>
                      </div>
                      <div
                        class="map-pin pin-poi pin-live"
                        :class="{ 'is-selected': isRouteSelected('星辰直播间') }"
                        style="top: 85%; left: 50%"
                        @click.stop="navigateToPin(50, 85, '星辰直播间')"
                      >
                        <span class="pin-icon">🎬</span><span class="pin-label">星辰直播间</span>
                      </div>
                      
                      <div
                        class="map-pin pin-poi pin-clinic"
                        :class="{ 'is-selected': isRouteSelected('优生学妇科诊所') }"
                        style="top: 65%; left: 40%"
                        @click.stop="navigateToPin(40, 65, '优生学妇科诊所')"
                      >
                        <span class="pin-icon">🏥</span><span class="pin-label">优生学妇科诊所</span>
                      </div>
                      <div
                        class="map-pin pin-poi pin-massage"
                        :class="{ 'is-selected': isRouteSelected('QoS按摩店') }"
                        style="top: 75%; left: 45%"
                        @click.stop="navigateToPin(45, 75, 'QoS按摩店')"
                      >
                        <span class="pin-icon">💆</span><span class="pin-label">QoS按摩店</span>
                      </div>
                      <div
                        class="map-pin pin-poi pin-fortune"
                        :class="{ 'is-selected': isRouteSelected('狐狸巫女占卜店') }"
                        style="top: 90%; left: 65%"
                        @click.stop="navigateToPin(65, 90, '狐狸巫女占卜店')"
                      >
                        <span class="pin-icon">🔮</span><span class="pin-label">狐狸巫女占卜店</span>
                      </div>

                      <div
                        class="map-pin pin-poi pin-crepe"
                        :class="{ 'is-selected': isRouteSelected('玛莉欧可丽饼店') }"
                        style="top: 52%; left: 80%"
                        @click.stop="navigateToPin(80, 52, '玛莉欧可丽饼店')"
                      >
                        <span class="pin-icon">🥞</span><span class="pin-label">玛莉欧可丽饼店</span>
                      </div>
                      <div
                        class="map-pin pin-poi pin-nail"
                        :class="{ 'is-selected': isRouteSelected('黑桃美甲沙龙') }"
                        style="top: 68%; left: 85%"
                        @click.stop="navigateToPin(85, 68, '黑桃美甲沙龙')"
                      >
                        <span class="pin-icon">💅</span><span class="pin-label">黑桃美甲沙龙</span>
                      </div>
                      <div
                        class="map-pin pin-poi pin-dorm"
                        :class="{ 'is-selected': isRouteSelected('太阳之家寄宿公寓') }"
                        style="top: 78%; left: 90%"
                        @click.stop="navigateToPin(90, 78, '太阳之家寄宿公寓')"
                      >
                        <span class="pin-icon">🏢</span><span class="pin-label">太阳之家寄宿公寓</span>
                      </div>
                      <div
                        class="map-pin pin-poi pin-feminist"
                        :class="{ 'is-selected': isRouteSelected('新黎明女权协会') }"
                        style="top: 58%; left: 92%"
                        @click.stop="navigateToPin(92, 58, '新黎明女权协会')"
                      >
                        <span class="pin-icon">♀</span><span class="pin-label">新黎明女权协会</span>
                      </div>
                    </div>
                  </div>

                  <div class="map-controls">
                    <button class="map-btn map-btn-compass" @click="resetMap">🧭</button>
                    <button class="map-btn" @click="zoomMap(0.2)">⊕</button>
                    <button class="map-btn" @click="zoomMap(-0.2)">⊖</button>
                    <button class="map-btn current-loc" @click="centerOnPin(30, 20)">◎</button>
                  </div>
                  
                  <div class="map-bottom-sheet">
                    <div class="sheet-handle"></div>
                    <div class="sheet-destination-chip">
                      <span class="sheet-destination-dot"></span>
                      <span>当前目的地</span>
                      <strong>{{ routeDestName }}</strong>
                    </div>
                    <div class="sheet-title">{{ routeDuration }} ({{ routeDistance }})</div>
                    <div class="sheet-subtitle">{{ routeSubtitle }}</div>
                    <button class="sheet-start-btn" @click="startNavigation">开始导航</button>
                  </div>
                </div>
              </div>
            </article>
          ...
          <section v-else-if="activeTab === 'hypnosis'" class="tab-page tamamo-page word-spirit-page">
            <div class="tamamo-shoji-shadow" aria-hidden="true"></div>
            <div class="tamamo-foxfire-layer" aria-hidden="true">
              <span v-for="i in 12" :key="`ws-foxfire-${i}`" class="foxfire-dot"></span>
            </div>

            <article class="tamamo-hero word-spirit-hero">
              <div class="fox-ear fox-ear-left" aria-hidden="true"></div>
              <div class="fox-ear fox-ear-right" aria-hidden="true"></div>
              <div class="tamamo-caster-circle" aria-hidden="true"></div>
              <span class="tamamo-torii-frame top" aria-hidden="true"></span>
              ...
              <div class="tamamo-hero-copy">
                <button class="word-spirit-back-btn" @click="activeTab = 'forum'">← 返回</button>
                <div class="tamamo-headline-row" style="margin-top: 8px;">
                  <div>
                    <p class="tamamo-eyebrow">Fate / Caster · 御前言灵录</p>
                    <h2>言灵</h2>
                  </div>
                  <span class="tamamo-phase-badge">咒相缔结中</span>
                </div>
                <div class="tamamo-mikon-banner" style="margin-top: 4px;">
                  <span class="mikon-icon" aria-hidden="true"></span>
                  Mikon~ 妾身的术法，将为您重塑世界之理
                  <span class="mikon-icon" aria-hidden="true"></span>
                </div>
                <div class="word-spirit-target-panel">
                  <div class="word-spirit-target-summary">
                    <p class="word-spirit-target-label">当前术式目标</p>
                    <strong>{{ hypnosisTargetName || '未选定目标' }}</strong>
                    <span v-if="hypnosisCurrentStatus" class="word-spirit-target-chip">{{ hypnosisCurrentStatus }}</span>
                  </div>

                  <div class="word-spirit-group-tabs">
                    <button
                      class="word-spirit-group-tab"
                      :class="{ active: hypnosisSelectedGroup === 'conqueror' }"
                      :disabled="hypnosisConquerorTargets.length === 0"
                      @click="selectHypnosisGroup('conqueror')"
                    >
                      征服者
                      <span>{{ hypnosisConquerorTargets.length }}</span>
                    </button>
                    <button
                      class="word-spirit-group-tab"
                      :class="{ active: hypnosisSelectedGroup === 'conquered' }"
                      :disabled="hypnosisConqueredTargets.length === 0"
                      @click="selectHypnosisGroup('conquered')"
                    >
                      被征服者
                      <span>{{ hypnosisConqueredTargets.length }}</span>
                    </button>
                  </div>

                  <div v-if="hypnosisCurrentTargets.length > 0" class="word-spirit-target-tabs">
                    <button
                      v-for="target in hypnosisCurrentTargets"
                      :key="target.id"
                      class="word-spirit-target-tab"
                      :class="{ active: hypnosisCurrentTarget?.id === target.id }"
                      @click="selectHypnosisTarget(target)"
                    >
                      {{ target.name }}
                    </button>
                  </div>

                  <div class="word-spirit-effect-panel">
                    <div class="word-spirit-effect-head">
                      <template v-if="hypnosisSelectedEntry">
                        <button class="word-spirit-effect-back" @click="closeHypnosisEntry">← 返回效果列表</button>
                        <div class="word-spirit-effect-head-actions">
                          <span>详情页</span>
                          <button
                            class="word-spirit-effect-delete"
                            @click="deleteHypnosisEntry(hypnosisSelectedEntry.name)"
                          >
                            删除
                          </button>
                        </div>
                      ...
                      <template v-else>
                        <p>催眠效果</p>
                        <span>{{ hypnosisCurrentEntries.length }} 项</span>
                      </template>
                    </div>

                    <div v-if="hypnosisCurrentEntries.length > 0">
                      <article v-if="hypnosisSelectedEntry" class="word-spirit-effect-item word-spirit-effect-detail">
                        <strong>{{ hypnosisSelectedEntry.name }}</strong>
                        <p>{{ hypnosisSelectedEntry.effect }}</p>
                      </article>

                      <div v-else class="word-spirit-effect-list">
                        <article
                          v-for="entry in hypnosisCurrentEntries"
                          :key="`${hypnosisCurrentTarget?.id}-${entry.name}`"
                          class="word-spirit-effect-item word-spirit-effect-trigger"
                        >
                          <button class="word-spirit-effect-open" @click="openHypnosisEntry(entry.name)">
                            <strong>{{ entry.name }}</strong>
                            <span>点击查看具体效果</span>
                          </button>
                          <button
                            class="word-spirit-effect-delete"
                            @click="deleteHypnosisEntry(entry.name)"
                          >
                            删除
                          </button>
                        </article>
                    ...
                    <p v-else class="word-spirit-effect-empty">该目标暂无催眠效果记录</p>
                  </div>
                </div>
              </div>
            </article>

            <article class="tamamo-gm-grid word-spirit-grid">
              <section
                v-for="(func, index) in hypnosisFunctions"
                :key="func.id"
                class="tamamo-card tamamo-scroll word-spirit-card"
                @click="onHypnosisCardClick(func)"
              >
                <div class="scroll-rod scroll-rod-top" aria-hidden="true"></div>
                <header class="tamamo-card-header">
                  <div>
                    <p class="tamamo-card-overline">{{ ['☯','✦','◉','⚸','卍','◎'][index % 6] }} 言灵之相</p>
                    <h3>{{ func.name }}</h3>
                  </div>
                  <span class="word-spirit-card-icon">{{ func.icon }}</span>
                </header>
                <div class="tamamo-card-body">
                  <p class="word-spirit-card-desc">{{ func.desc }}</p>
                </div>
                <div class="scroll-rod scroll-rod-bottom" aria-hidden="true"></div>
              </section>
            </article>
        ...
        <footer class="bottom-tabs">
          ...
          <button class="nav-btn" :class="{ active: activeTab === 'map' }" @click="activeTab = 'map'">
            地图导航
          </button>
        </footer>
      </div>
    ...
    <div v-if="selectedContact" class="overlay" @click.self="selectedContact = null">
      ...
      >
        ...
        <button class="send-btn word-spirit-entry-btn" @click="enterHypnosisFromContact">✦ 言灵术式</button>
      </section>
    </div>
    ...
    <div v-if="selectedPerson" class="overlay" @click.self="selectedPerson = null">
      <section class="modal">
        ...
        <button class="send-btn word-spirit-entry-btn" @click="enterHypnosisFromPerson">✦ 言灵术式</button>
      </section>
    </div>
    ...
    <div v-if="hypnosisModal" class="overlay hypnosis-modal-overlay" @click.self="hypnosisModal = null">
      <section class="tamamo-card tamamo-scroll word-spirit-modal" @click.stop>
        <div class="scroll-rod scroll-rod-top" aria-hidden="true"></div>
        <header class="tamamo-card-header">
          <div>
            <p class="tamamo-card-overline">⛩ 御前律令</p>
            <h3>{{ hypnosisModal.name }}</h3>
          </div>
          <button class="word-spirit-modal-close" @click="hypnosisModal = null">×</button>
        </header>

        <div class="tamamo-card-body">
          <div class="word-spirit-modal-icon">{{ hypnosisModal.icon }}</div>
          <p class="word-spirit-modal-desc">{{ hypnosisModal.desc }}</p>
          <p v-if="hypnosisTargetName" class="word-spirit-modal-target">施术目标：{{ hypnosisTargetName }}</p>

          <div v-if="hypnosisModal.id === 'sensitive'" class="word-spirit-modal-form">
            <label class="tamamo-label">感知阈值放大倍率</label>
            <div class="word-spirit-slider-row">
              <input
                v-model.number="hypnosisSensitiveMultiplier"
                type="range"
                min="1"
                max="100"
                step="1"
                class="word-spirit-slider"
              />
              <span class="word-spirit-slider-value">×{{ hypnosisSensitiveMultiplier }}</span>
            </div>
            <p class="word-spirit-form-hint">当前结界律令：将目标五感放大 {{ hypnosisSensitiveMultiplier }} 倍</p>
          </div>

          <div v-else class="word-spirit-modal-form">
            <label class="tamamo-label">{{ hypnosisModalInputLabel }}</label>
            <textarea
              v-model="hypnosisInputText"
              class="tamamo-input tamamo-textarea word-spirit-textarea"
              :placeholder="hypnosisModalPlaceholder"
              rows="4"
            ></textarea>
          </div>

          <button class="tamamo-direct-btn word-spirit-submit-btn" :disabled="!isHypnosisFormValid" @click="submitHypnosisCommand">
            执行言灵术式
          </button>
        </div>
        <div class="scroll-rod scroll-rod-bottom" aria-hidden="true"></div>
...
type MapRoutePreset = {
  ...
  distance: string;
  subtitle: string;
  transportModes: MapTransportChip[];
  startPrompt: string;
};
...
type HypnosisTargetGroup = 'conqueror' | 'conquered';

type HypnosisTargetItem = {
  id: string;
  key: string;
  name: string;
  group: HypnosisTargetGroup;
  status: string;
  hypnosisPath: Array<string>;
  hypnosisEntries: Array<{ name: string; effect: string }>;
};
...
// Map state
const isMapHeaderCollapsed = ref(false);
const mapScale = ref(1);
const mapOffsetX = ref(0);
const mapOffsetY = ref(0);
const mapIsDragging = ref(false);
const mapDragStartX = ref(0);
const mapDragStartY = ref(0);

function startMapDrag(e: MouseEvent | TouchEvent) {
  if ('button' in e && e.button !== 0) return; // Only allow left click
  mapIsDragging.value = true;
  const clientX = 'touches' in e ? e.touches[0].clientX : e.clientX;
  const clientY = 'touches' in e ? e.touches[0].clientY : e.clientY;
  mapDragStartX.value = clientX - mapOffsetX.value;
  mapDragStartY.value = clientY - mapOffsetY.value;
}

function onMapDrag(e: MouseEvent | TouchEvent) {
  if (!mapIsDragging.value) return;
  const clientX = 'touches' in e ? e.touches[0].clientX : e.clientX;
  const clientY = 'touches' in e ? e.touches[0].clientY : e.clientY;
  mapOffsetX.value = clientX - mapDragStartX.value;
  mapOffsetY.value = clientY - mapDragStartY.value;
}
...
function zoomMap(delta: number) {
  const newScale = Math.max(0.3, Math.min(3, mapScale.value + delta));
...
function returnToHomeFromMap() {
  activeTab.value = 'forum';
}

function handleBackdropReturnHome() {
  if (activeTab.value !== 'map') return;
  if (selectedContact.value || selectedPerson.value || activePanel.value || imagePreview.value) return;
  returnToHomeFromMap();
}

const mapViewRef = ref<HTMLElement | null>(null);
const defaultRoutePreset: MapRoutePreset = {
  x: 65,
  y: 65,
  duration: '9 分钟',
  distance: '3.8 公里',
  subtitle: '主干道畅通，适合快速踩点',
  transportModes: [
    { icon: '🚗', time: '9分', active: true },
    { icon: '🚶', time: '28分' },
    { icon: '🚇', time: '14分' },
  ],
  startPrompt: '前往商业街区',
...
const mapRoutePresets: Record<string, MapRoutePreset> = {
  商业街区: defaultRoutePreset,
  天王寺家: {
    x: 30,
    y: 20,
    duration: '2 分钟',
    distance: '0.6 公里',
    subtitle: '已回到熟悉街区，沿住宅支路即可抵达',
    transportModes: [
      { icon: '🚗', time: '2分', active: true },
      { icon: '🚶', time: '9分' },
      { icon: '🚇', time: '6分' },
    ],
    startPrompt: '前往天王寺家',
  ...
  大神家: {
    ...
    duration: '5 分钟',
    distance: '1.4 公里',
    subtitle: '住宅区道路通畅，转过樱花街角就能到',
    transportModes: [
      { icon: '🚗', time: '5分', active: true },
      { icon: '🚶', time: '18分' },
      { icon: '🚇', time: '11分' },
    ],
    startPrompt: '前往大神家',
  ...
  迦南家: {
    ...
    duration: '6 分钟',
    distance: '1.8 公里',
    subtitle: '建议穿过北侧道路，避开校门口的人流',
    transportModes: [
      { icon: '🚗', time: '6分', active: true },
      { icon: '🚶', time: '21分' },
      { icon: '🚇', time: '10分' },
    ],
    startPrompt: '前往迦南家',
  ...
  一条家: {
    ...
    duration: '8 分钟',
    distance: '2.3 公里',
    subtitle: '沿辅路南下即可，路口监控较少',
    transportModes: [
      { icon: '🚗', time: '8分', active: true },
      { icon: '🚶', time: '26分' },
      { icon: '🚇', time: '14分' },
    ],
    startPrompt: '前往一条家',
  ...
  黑桃咖啡厅: {
    ...
    duration: '10 分钟',
    distance: '4.1 公里',
    subtitle: '主路霓虹段最顺畅，适合快速切入商业街',
    transportModes: [
      { icon: '🚗', time: '10分', active: true },
      { icon: '🚶', time: '34分' },
      { icon: '🚇', time: '16分' },
    ],
    startPrompt: '前往黑桃咖啡厅',
  ...
  太阳女性健身房: {
    ...
    duration: '13 分钟',
    distance: '5.8 公里',
    subtitle: '东侧高架稍有拥堵，预计中段减速',
    transportModes: [
      { icon: '🚗', time: '13分', active: true },
      { icon: '🚶', time: '47分' },
      { icon: '🚇', time: '21分' },
    ],
    startPrompt: '前往太阳女性健身房',
  },
  黑日教堂: {
    x: 60,
    y: 70,
    duration: '11 分钟',
    distance: '4.9 公里',
    subtitle: '走中央干道更快，夜间路灯覆盖完整',
    transportModes: [
      { icon: '🚗', time: '11分', active: true },
      { icon: '🚶', time: '39分' },
      { icon: '🚇', time: '18分' },
    ],
    startPrompt: '前往黑日教堂',
  ...
  黑桃夜总会: {
    ...
    duration: '14 分钟',
    distance: '6.4 公里',
    subtitle: '南段霓虹大道车流密集，建议保持当前路线',
    transportModes: [
      { icon: '🚗', time: '14分', active: true },
      { icon: '🚶', time: '52分' },
      { icon: '🚇', time: '23分' },
    ],
    startPrompt: '前往黑桃夜总会',
  ...
  星辰直播间: {
    ...
    duration: '15 分钟',
    distance: '6.8 公里',
    subtitle: '终点位于南区，最后一段需要穿过人行街',
    transportModes: [
      { icon: '🚗', time: '15分', active: true },
      { icon: '🚶', time: '56分' },
      { icon: '🚇', time: '25分' },
    ],
    startPrompt: '前往星辰直播间',
  ...
  优生学妇科诊所: {
    ...
    subtitle: '最快路线，路况良好',
    transportModes: [
      { icon: '🚗', time: '12分', active: true },
      { icon: '🚶', time: '45分' },
      { icon: '🚇', time: '20分' },
    ],
    startPrompt: '前往优生学妇科诊所',
  },
  QoS按摩店: {
    x: 45,
    y: 75,
    duration: '13 分钟',
    distance: '5.7 公里',
    subtitle: '穿过南侧街区后直达，沿路监控较密集',
    transportModes: [
      { icon: '🚗', time: '13分', active: true },
      { icon: '🚶', time: '48分' },
      { icon: '🚇', time: '22分' },
    ],
    startPrompt: '前往QoS按摩店',
  },
  狐狸巫女占卜店: {
    x: 65,
    y: 90,
    duration: '17 分钟',
    distance: '7.5 公里',
    subtitle: '最南端路线较长，但中途无明显拥堵',
    transportModes: [
      { icon: '🚗', time: '17分', active: true },
      { icon: '🚶', time: '62分' },
      { icon: '🚇', time: '28分' },
    ],
    startPrompt: '前往狐狸巫女占卜店',
  },
  玛莉欧可丽饼店: {
    x: 80,
    y: 52,
    duration: '14 分钟',
    distance: '6.1 公里',
    subtitle: '推荐沿东区主干线前进，红灯较少',
    transportModes: [
      { icon: '🚗', time: '14分', active: true },
      { icon: '🚶', time: '50分' },
      { icon: '🚇', time: '21分' },
    ],
    startPrompt: '前往玛莉欧可丽饼店',
  },
  黑桃美甲沙龙: {
    x: 85,
    y: 68,
    duration: '16 分钟',
    distance: '6.9 公里',
    subtitle: '商业区中后段稍慢，建议保持高架路线',
    transportModes: [
      { icon: '🚗', time: '16分', active: true },
      { icon: '🚶', time: '58分' },
      { icon: '🚇', time: '26分' },
    ],
    startPrompt: '前往黑桃美甲沙龙',
  },
  太阳之家寄宿公寓: {
    x: 90,
    y: 78,
    duration: '18 分钟',
    distance: '7.8 公里',
    subtitle: '东南街区路段较长，末段通行速度偏慢',
    transportModes: [
      { icon: '🚗', time: '18分', active: true },
      { icon: '🚶', time: '66分' },
      { icon: '🚇', time: '31分' },
    ],
    startPrompt: '前往太阳之家寄宿公寓',
  },
  新黎明女权协会: {
    x: 92,
    y: 58,
    duration: '15 分钟',
    distance: '6.5 公里',
    subtitle: '东区线路稳定，沿线电子路牌状态正常',
    transportModes: [
      { icon: '🚗', time: '15分', active: true },
      { icon: '🚶', time: '54分' },
      { icon: '🚇', time: '24分' },
    ],
    startPrompt: '前往新黎明女权协会',
  },
};

function centerOnPin(xPercent: number, yPercent: number) {
  let mapWidth = 375;
  let mapHeight = 667;
  ...
  const targetX = (xPercent / 100) * mapWidth;
  const targetY = (yPercent / 100) * mapHeight;
  
  mapOffsetX.value = (mapWidth / 2) - targetX * mapScale.value;
  mapOffsetY.value = (mapHeight / 2) - targetY * mapScale.value;
  isMapHeaderCollapsed.value = true;
}

const routeDestName = ref('商业街区');
const routeDestX = ref(defaultRoutePreset.x);
const routeDestY = ref(defaultRoutePreset.y);
const routeDuration = ref(defaultRoutePreset.duration);
const routeDistance = ref(defaultRoutePreset.distance);
const routeSubtitle = ref(defaultRoutePreset.subtitle);
const routeTransportModes = ref<MapTransportChip[]>(defaultRoutePreset.transportModes.map(item => ({ ...item })));
const routeStartPrompt = ref(defaultRoutePreset.startPrompt);

const routePathD = computed(() => {
  const sx = 30; // 固定起点: 天王寺家
  const sy = 20;
  const ex = routeDestX.value;
  const ey = routeDestY.value;
...
function applyRoutePreset(name: string, xPercent: number, yPercent: number) {
  const preset = mapRoutePresets[name] ?? {
    ...defaultRoutePreset,
    x: xPercent,
    y: yPercent,
    startPrompt: `前往${name}`,
  };
  routeDestX.value = preset.x;
  routeDestY.value = preset.y;
  routeDestName.value = name;
  routeDuration.value = preset.duration;
  routeDistance.value = preset.distance;
  routeSubtitle.value = preset.subtitle;
  routeTransportModes.value = preset.transportModes.map(item => ({ ...item }));
  routeStartPrompt.value = preset.startPrompt;
}

function isRouteSelected(name: string) {
  return routeDestName.value === name;
}

function navigateToPin(xPercent: number, yPercent: number, name: string) {
  applyRoutePreset(name, xPercent, yPercent);
  centerOnPin(routeDestX.value, routeDestY.value);
}

function getTavernHostWindow() {
  const candidates = [window.top, window.parent, window];
  for (const candidate of candidates) {
    try {
      if (candidate?.document) return candidate as Window & typeof globalThis;
    } catch {
...
function syncTavernTextarea(hostWindow: Window & typeof globalThis, value: string) {
  const textarea = hostWindow.document.querySelector<HTMLTextAreaElement>('#send_textarea');
  if (!textarea) return null;

  const valueSetter = Object.getOwnPropertyDescriptor(hostWindow.HTMLTextAreaElement.prototype, 'value')?.set;
  ...
  textarea.dispatchEvent(new hostWindow.KeyboardEvent('keyup', { bubbles: true, key: 'Process' }));
  textarea.focus();

  return textarea;
}

async function sendPromptToTavern(prompt: string) {
  const hostWindow = getTavernHostWindow();
  const textarea = syncTavernTextarea(hostWindow, prompt);
  if (textarea) return;
  throw new Error('Unable to locate SillyTavern input box');
}

async function startNavigation() {
  try {
    await sendPromptToTavern(routeStartPrompt.value);
    showToast(`已填入导航提示词：${routeStartPrompt.value}`);
  } catch (error) {
    console.error(error);
    showToast('导航提示词填入失败');
  }
...
// ── Hypnosis ──
const hypnosisPreferredTargetName = ref('');
const hypnosisSelectedGroup = ref<HypnosisTargetGroup>('conquered');
const hypnosisSelectedTargetId = ref('');
const hypnosisSelectedEntryName = ref('');
const hypnosisFunctions = [
  { id: 'unconscious', icon: '🌀', name: '言灵·无识侵透', desc: '潜入意识空洞，解除防御壁垒' },
  { id: 'body', icon: '🎭', name: '言灵·肉体傀儡', desc: '剥夺躯体自治权，建立支配链路' },
  { id: 'sensitive', icon: '⚡', name: '言灵·感官狂化', desc: '重布神经回路，突破感知阈值' },
  { id: 'common', icon: '🧠', name: '言灵·理之覆写', desc: '粉碎旧有常识，植入界定之理' },
  { id: 'memory', icon: '💫', name: '言灵·记忆编织', desc: '剪裁过去痕迹，纺织虚妄羁绊' },
  { id: 'persona', icon: '👁', name: '言灵·魂之刻印', desc: '熔毁人格核心，纂写绝对忠诚' },
];

const hypnosisTargets = computed<HypnosisTargetItem[]>(() => {
  const targets: HypnosisTargetItem[] = [];
  const conquerorState = get(statData.value, '征服者状态', {}) as Record<string, unknown>;
  const conqueredState = get(statData.value, '被征服者状态', {}) as Record<string, unknown>;

  Object.entries(conquerorState).forEach(([key, value]) => {
    const name = String(get(value, '姓名', key)).trim() || key;
    const hypnosisEntries = Object.entries(get(value, '催眠列表', {}) as Record<string, unknown>)
      .map(([entryName, effect]) => ({
        name: entryName,
        effect: String(effect ?? '').trim(),
      }))
      .filter(entry => entry.name.trim() || entry.effect);
    ...
    targets.push({
      ...
      name,
      group: 'conqueror',
      status: String(get(value, '当前状态', '潜伏中')).trim(),
      hypnosisPath: ['征服者状态', key, '催眠列表'],
      hypnosisEntries,
    });
  });
  ...
  Object.entries(conqueredState).forEach(([key, value]) => {
    ...
    targets.push({
      ...
      name,
      group: 'conquered',
      status: formatValue(get(value, '当前状态', get(value, '临时状态', '待观测')), '待观测'),
      hypnosisPath: ['被征服者状态', key, '催眠列表'],
      hypnosisEntries,
    });
  });
...
const hypnosisConquerorTargets = computed(() =>
  hypnosisTargets.value.filter(target => target.group === 'conqueror'),
);

const hypnosisConqueredTargets = computed(() =>
  hypnosisTargets.value.filter(target => target.group === 'conquered'),
);

const hypnosisCurrentTargets = computed(() => {
  if (hypnosisSelectedGroup.value === 'conqueror') return hypnosisConquerorTargets.value;
  return hypnosisConqueredTargets.value;
});

const hypnosisCurrentTarget = computed(() => {
  const activeTargets = hypnosisCurrentTargets.value.length > 0 ? hypnosisCurrentTargets.value : hypnosisTargets.value;
  if (activeTargets.length === 0) return null;

  const matchedById = activeTargets.find(target => target.id === hypnosisSelectedTargetId.value);
  if (matchedById) return matchedById;

  const preferredName = normalizeCharacterName(hypnosisPreferredTargetName.value);
  if (preferredName) {
    const matchedByName =
      activeTargets.find(target => normalizeCharacterName(target.name) === preferredName) ||
      activeTargets.find(target => normalizeCharacterName(target.key) === preferredName);
...
const hypnosisTargetName = computed(() => hypnosisCurrentTarget.value?.name ?? '');
const hypnosisCurrentStatus = computed(() => hypnosisCurrentTarget.value?.status ?? '');
const hypnosisCurrentEntries = computed(() => hypnosisCurrentTarget.value?.hypnosisEntries ?? []);
const hypnosisSelectedEntry = computed(() =>
  hypnosisCurrentEntries.value.find(entry => entry.name === hypnosisSelectedEntryName.value) ?? null,
);

function openHypnosisEntry(name: string) {
  hypnosisSelectedEntryName.value = name;
}

function closeHypnosisEntry() {
  hypnosisSelectedEntryName.value = '';
}

function selectHypnosisTarget(target: HypnosisTargetItem) {
  hypnosisSelectedGroup.value = target.group;
  hypnosisSelectedTargetId.value = target.id;
  hypnosisPreferredTargetName.value = target.name;
  closeHypnosisEntry();
}

function selectHypnosisGroup(group: HypnosisTargetGroup) {
  hypnosisSelectedGroup.value = group;
  const groupTargets = group === 'conqueror' ? hypnosisConquerorTargets.value : hypnosisConqueredTargets.value;
  const preferredName = normalizeCharacterName(hypnosisPreferredTargetName.value);
  const nextTarget =
    groupTargets.find(target => normalizeCharacterName(target.name) === preferredName) ||
    groupTargets.find(target => normalizeCharacterName(target.key) === preferredName) ||
    groupTargets[0];

  hypnosisSelectedTargetId.value = nextTarget?.id ?? '';
  if (nextTarget) {
    hypnosisPreferredTargetName.value = nextTarget.name;
  }
  closeHypnosisEntry();
}

function setHypnosisTargetByName(name: string, preferredGroup?: HypnosisTargetGroup) {
  const normalizedName = normalizeCharacterName(name);
  hypnosisPreferredTargetName.value = String(name).trim();
  if (preferredGroup) {
    hypnosisSelectedGroup.value = preferredGroup;
  }

  const candidateTargets = preferredGroup
    ? hypnosisTargets.value.filter(target => target.group === preferredGroup)
    : hypnosisTargets.value;

  const matchedTarget =
    candidateTargets.find(target => normalizeCharacterName(target.name) === normalizedName) ||
    candidateTargets.find(target => normalizeCharacterName(target.key) === normalizedName) ||
    hypnosisTargets.value.find(target => normalizeCharacterName(target.name) === normalizedName) ||
    hypnosisTargets.value.find(target => normalizeCharacterName(target.key) === normalizedName);

  hypnosisSelectedTargetId.value = matchedTarget?.id ?? '';
  if (matchedTarget) {
    hypnosisSelectedGroup.value = matchedTarget.group;
    hypnosisPreferredTargetName.value = matchedTarget.name;
  }
  closeHypnosisEntry();
}

function enterHypnosisFromContact() {
  if (selectedContact.value) {
    setHypnosisTargetByName(selectedContact.value.name, 'conquered');
    selectedContact.value = null;
  }
  activeTab.value = 'hypnosis';
}

function enterHypnosisFromPerson() {
  if (selectedPerson.value) {
    setHypnosisTargetByName(selectedPerson.value.name, 'conqueror');
    selectedPerson.value = null;
  }
  activeTab.value = 'hypnosis';
}

type HypnosisFunc = { id: string; icon: string; name: string; desc: string };
const hypnosisModal = ref<HypnosisFunc | null>(null);
const hypnosisInputText = ref('');
const hypnosisSensitiveMultiplier = ref(2);

const hypnosisModalInputLabel = computed(() => {
  const m = hypnosisModal.value;
  if (!m) return '';
  ...
  return labels[m.id] ?? '术式律令内容';
});

const hypnosisModalPlaceholder = computed(() => {
  const m = hypnosisModal.value;
  if (!m) return '';
  const placeholders: Record<string, string> = {
    unconscious: '构筑潜意识律令……例如：“当听到铃响，你将舍弃思考服从命令”',
    body: '构筑躯体支配律令……例如：“从此刻起，身体不由自主摆出指定姿态”',
    common: '撰写新世之理……例如：“在此世的法则中，向御主献上一切是绝对真理”',
    memory: '纺织不存在的过去……例如：“你的记忆深处，唯有自幼与妾身结缘的身影”',
    persona: '篆刻新魂之印……例如：“抹去反叛意志，化作全心奉献的永恒忠犬”',
  };
  return placeholders[m.id] ?? '输入术式律令……';
});

const isHypnosisFormValid = computed(() => {
  const m = hypnosisModal.value;
  if (!m) return false;
  if (m.id === 'sensitive') return hypnosisSensitiveMultiplier.value >= 1;
  return hypnosisInputText.value.trim().length > 0;
});

function onHypnosisCardClick(func: HypnosisFunc) {
  hypnosisInputText.value = '';
  hypnosisSensitiveMultiplier.value = 2;
  hypnosisModal.value = func;
}

function buildHypnosisRecord(func: HypnosisFunc) {
  const baseNameMap: Record<HypnosisFunc['id'], string> = {
    unconscious: '无意识催眠',
    body: '肉体操控',
    sensitive: '敏感度调整',
    common: '篡改常识',
  ...
  const baseName = baseNameMap[func.id] ?? func.name;
  const effectText = func.id === 'sensitive'
    ? String(hypnosisSensitiveMultiplier.value).trim()
    : hypnosisInputText.value.trim();

  return {
    name: baseName,
    effect: effectText || '未设定',
  };
}

function writeHypnosisRecord(target: HypnosisTargetItem, entry: { name: string; effect: string }) {
  const nextStatData = cloneDeep(statData.value);
  const currentList = get(nextStatData, target.hypnosisPath, {}) as Record<string, string>;
  set(nextStatData, target.hypnosisPath, {
    ...currentList,
    [entry.name]: entry.effect,
  });
  writePersistentStatData(nextStatData);
}

function deleteHypnosisEntry(name: string) {
  const target = hypnosisCurrentTarget.value;
  if (!target) {
    showToast('未找到可操作目标');
    return;
  }
  ...
  set(nextStatData, target.hypnosisPath, currentList);
  writePersistentStatData(nextStatData);

  if (hypnosisSelectedEntryName.value === name) {
    closeHypnosisEntry();
  }
...
async function submitHypnosisCommand() {
  const m = hypnosisModal.value;
  if (!m) return;
  const currentTarget = hypnosisCurrentTarget.value;
  const target = hypnosisTargetName.value || '未选定目标';
  const hypnosisRecord = buildHypnosisRecord(m);
  let prompt = '';
  if (m.id === 'sensitive') {
    prompt = `[言灵术式：感官狂化] 对象：${target} | 感知放大：×${hypnosisSensitiveMultiplier.value}`;
  } else {
    prompt = `[${m.name}] 对象：${target} | 律令内容：${hypnosisInputText.value.trim()}`;
  }
  try {
    await sendPromptToTavern(prompt);
    if (currentTarget) {
      writeHypnosisRecord(currentTarget, hypnosisRecord);
      openHypnosisEntry(hypnosisRecord.name);
    }
    showToast(`${m.name} 已投送结界`);
    hypnosisModal.value = null;
  } catch {
    showToast('术式释放失败，法力紊乱');
  }
}
...
const CHAT_VARIABLE_SCOPE: VariableScope = { type: 'chat' };

function getCurrentMessageScope(): VariableScope {
  return {
    type: 'message',
...
function asStatData(value: unknown): Record<string, unknown> {
...
function getVariableApi() {
  ...
  const getAllVariablesFn = tavernHelper?.getAllVariables ?? fallback.getAllVariables;
  ...
  return {
    ...
    getAllVariables: typeof getAllVariablesFn === 'function' ? getAllVariablesFn : undefined,
  };
}

function readPersistedStatData() {
  const variableApi = getVariableApi();
  const chatVariables = variableApi.getVariables(CHAT_VARIABLE_SCOPE);
  const chatStatData = asStatData(get(chatVariables, 'stat_data'));
  const messageVariables = variableApi.getVariables(getCurrentMessageScope());
  const messageStatData = asStatData(get(messageVariables, 'stat_data'));

  if (Object.keys(chatStatData).length === 0) {
    return messageStatData;
  }

  if (Object.keys(messageStatData).length === 0) {
    return chatStatData;
  }

  return _.merge(cloneDeep(chatStatData), messageStatData);
}

function reloadVariables(showSuccess = false) {
  try {
    statData.value = readPersistedStatData();
...
const malePlayerAvatarUrl = 'https://CDN.jsdelivr.net/gh/enterprise20020924-web/-/llm1/男u.png';
...
const isPlayerMale = computed(() => {
  const normalized = playerGenderText.value.trim().toLowerCase();
  return ['男', '男性', 'male', 'man'].includes(normalized);
});
...
const playerAvatarUrl = computed(() =>
  isPlayerMale.value ? malePlayerAvatarUrl : buildCorruptionImageUrl(playerImageName.value, playerCorruptionText.value),
);
const playerFullImageUrl = computed(() => {
  if (isPlayerMale.value) return malePlayerAvatarUrl;
...
function writePersistentStatData(nextStatData: Record<string, unknown>) {
  const variableApi = getVariableApi();
  const chatVariables = variableApi.getVariables(CHAT_VARIABLE_SCOPE);
  set(chatVariables, 'stat_data', nextStatData);
  variableApi.replaceVariables(chatVariables, CHAT_VARIABLE_SCOPE);

  try {
    const messageScope = getCurrentMessageScope();
    const messageVariables = variableApi.getVariables(messageScope);
    set(messageVariables, 'stat_data', nextStatData);
    variableApi.replaceVariables(messageVariables, messageScope);
  } catch (error) {
    console.warn('[深渊APP] 同步当前楼层变量失败，将继续保留聊天级持久写入', error);
  }
...
/* ═══════════════════════════════════════════
   WORD SPIRIT APP PAGE (Tamamo Style)
   ═══════════════════════════════════════════ */
.word-spirit-page {
  position: relative;
  overflow: hidden;
...
.word-spirit-back-btn:hover {
  background: rgba(201, 168, 76, 0.15);
  border-color: rgba(201, 168, 76, 0.8);
}

.word-spirit-target-panel {
  margin-top: 10px;
  padding: 12px 14px 14px;
  border-radius: 16px;
  background:
    linear-gradient(180deg, rgba(30, 18, 44, 0.82), rgba(20, 13, 31, 0.92)),
    radial-gradient(circle at top right, rgba(255, 120, 186, 0.18), transparent 42%);
...
.word-spirit-target-summary strong {
  font-size: 15px;
  color: #f7e9b6;
  letter-spacing: 0.02em;
}

.word-spirit-target-label {
  margin: 0;
  font-size: 11px;
  color: rgba(240, 230, 211, 0.62);
}

.word-spirit-target-chip {
  display: inline-flex;
  align-items: center;
  margin-left: auto;
  padding: 6px 12px;
  ...
  box-shadow: 0 0 10px rgba(155, 114, 207, 0.15);
  animation: spiritChipPulse 2s ease-in-out infinite;
}

@keyframes spiritChipPulse {
  0%, 100% { box-shadow: 0 0 6px rgba(155, 114, 207, 0.15); }
  50%      { box-shadow: 0 0 16px rgba(155, 114, 207, 0.35); }
}

.word-spirit-group-tabs,
.word-spirit-target-tabs {
  display: flex;
  gap: 8px;
  margin-top: 12px;
  flex-wrap: wrap;
...
.word-spirit-group-tab,
.word-spirit-target-tab {
...
.word-spirit-group-tab:hover:not(:disabled),
.word-spirit-target-tab:hover {
  transform: translateY(-1px);
  border-color: rgba(201, 168, 76, 0.48);
  box-shadow: 0 6px 18px rgba(0, 0, 0, 0.18);
}

.word-spirit-group-tab.active,
.word-spirit-target-tab.active {
  color: #f6e7a6;
  border-color: rgba(201, 168, 76, 0.68);
  background:
    linear-gradient(135deg, rgba(201, 168, 76, 0.22), rgba(255, 122, 188, 0.1)),
    rgba(20, 13, 31, 0.92);
  box-shadow: inset 0 0 18px rgba(201, 168, 76, 0.08);
}

.word-spirit-group-tab:disabled {
  opacity: 0.45;
  cursor: not-allowed;
}

.word-spirit-effect-panel {
  margin-top: 14px;
  padding: 12px;
  border-radius: 14px;
  background: rgba(10, 7, 16, 0.52);
  border: 1px solid rgba(255, 255, 255, 0.05);
}

.word-spirit-effect-head {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
...
.word-spirit-effect-head p,
.word-spirit-effect-head span {
  margin: 0;
}
...
.word-spirit-effect-head-actions {
  display: flex;
  align-items: center;
  gap: 10px;
}

.word-spirit-effect-back {
  border: 0;
  padding: 0;
  background: none;
  color: #f5dc93;
...
.word-spirit-effect-back:hover {
  color: #ffe8a7;
}

.word-spirit-effect-list {
  display: grid;
  gap: 10px;
}

.word-spirit-effect-item {
  display: flex;
  align-items: stretch;
  gap: 10px;
  width: 100%;
  ...
  border-radius: 12px;
  background:
    linear-gradient(180deg, rgba(48, 28, 70, 0.6), rgba(28, 16, 40, 0.78));
  border: 1px solid rgba(201, 168, 76, 0.14);
}

.word-spirit-effect-trigger {
  transition: transform 0.2s ease, border-color 0.2s ease, box-shadow 0.2s ease;
}

.word-spirit-effect-trigger:hover {
  transform: translateY(-1px);
  border-color: rgba(201, 168, 76, 0.32);
  box-shadow: 0 8px 18px rgba(0, 0, 0, 0.18);
}

.word-spirit-effect-open {
  flex: 1;
  border: 0;
  padding: 0;
  background: none;
...
.word-spirit-effect-trigger span {
  display: block;
  margin-top: 6px;
  font-size: 10px;
...
.word-spirit-effect-delete {
  align-self: flex-start;
  border: 1px solid rgba(255, 122, 188, 0.22);
  background: rgba(255, 122, 188, 0.08);
  color: #ffb3d0;
  border-radius: 999px;
  padding: 5px 10px;
  font-size: 10px;
...
.word-spirit-effect-delete:hover {
  border-color: rgba(255, 122, 188, 0.48);
  background: rgba(255, 122, 188, 0.16);
  color: #ffd7e7;
}

.word-spirit-effect-detail {
  display: block;
  min-height: 108px;
}

.word-spirit-effect-item strong {
  display: block;
  margin-bottom: 6px;
  font-size: 12px;
  color: #f3e7bc;
...
.word-spirit-effect-item p,
.word-spirit-effect-empty {
  margin: 0;
  font-size: 11px;
  line-height: 1.6;
...
.word-spirit-card:hover {
  transform: translateY(-2px);
  box-shadow: 0 6px 20px rgba(0, 0, 0, 0.6), inset 0 0 12px rgba(201, 168, 76, 0.15);
}

.word-spirit-card-icon {
  font-size: 24px;
  filter: drop-shadow(0 0 6px rgba(201, 168, 76, 0.4));
  transition: transform 0.3s;
}

.word-spirit-card:hover .word-spirit-card-icon {
  transform: scale(1.15) rotate(5deg);
}

.word-spirit-card-desc {
  font-size: 11px;
  line-height: 1.5;
...
/* ── entry button override ── */
.word-spirit-entry-btn {
  background:
    linear-gradient(135deg, rgba(201, 168, 76, 0.15), transparent 40%),
    linear-gradient(90deg, #5a3c14, #8b6420 50%, #a17a2a 100%) !important;
...
.word-spirit-entry-btn:hover {
  box-shadow: 0 6px 16px rgba(201, 168, 76, 0.25), inset 0 1px 0 rgba(255, 255, 255, 0.2) !important;
...
.word-spirit-modal {
  ...
  max-width: 380px;
  margin: auto;
  background: linear-gradient(180deg, rgba(20, 12, 28, 0.98), rgba(15, 10, 22, 0.98));
  box-shadow: 0 10px 40px rgba(0, 0, 0, 0.8), 0 0 20px rgba(201, 168, 76, 0.15);
  animation: hypModalIn 0.3s cubic-bezier(0.2, 0.8, 0.2, 1);
}

.word-spirit-modal-close {
  background: none;
  border: none;
  color: rgba(240, 230, 211, 0.5);
  font-size: 22px;
...
.word-spirit-modal-close:hover {
  color: #c9a84c;
}

.word-spirit-modal-icon {
  font-size: 48px;
  text-align: center;
  margin: 10px 0 12px;
  filter: drop-shadow(0 0 10px rgba(201, 168, 76, 0.4));
  animation: shintoPulse 3s ease-in-out infinite;
}

@keyframes shintoPulse {
  0%, 100% { transform: translateY(0) scale(1); }
  50%      { transform: translateY(-4px) scale(1.05); }
}

.word-spirit-modal-desc {
  font-size: 12px;
...
.word-spirit-modal-target {
  display: block;
  text-align: center;
  padding: 6px;
  background: rgba(155, 114, 207, 0.1);
  border: 1px dashed rgba(155, 114, 207, 0.4);
  border-radius: 4px;
  font-size: 12px;
  font-weight: 700;
  color: #e8a0bf;
...
.word-spirit-modal-form {
  margin-bottom: 20px;
}

.word-spirit-textarea {
  margin-top: 8px;
  min-height: 90px;
}

/* ── slider ── */
.word-spirit-slider-row {
  display: flex;
  align-items: center;
  gap: 12px;
  margin-top: 12px;
...
.word-spirit-slider::-webkit-slider-thumb {
  -webkit-appearance: none;
  appearance: none;
  width: 18px;
  height: 18px;
  border-radius: 2px;
  background: linear-gradient(135deg, #e8c86a, #a17a2a);
...
.word-spirit-slider::-webkit-slider-thumb:hover {
  transform: rotate(45deg) scale(1.2);
}

.word-spirit-slider-value {
  font-size: 18px;
  font-weight: 900;
  min-width: 50px;
  text-align: center;
...
.word-spirit-submit-btn:disabled {
  opacity: 0.4;
  cursor: not-allowed;
}
</style>
