      ...
      <div class="phone-screen">
        ...
        <section class="meta-bar">
          <div>
            ...
            <p class="meta-sub">剧情时刻：{{ worldTimeText }} ｜ 时钟：{{ worldGdiText }}</p>
          </div>
            ...
            <article class="panel-card interaction-overview">
              ...
              <div class="overview-grid">
                ...
                <div class="overview-item">
                  ...
                  <p class="value">{{ playerStatusText }}</p>
                </div>
                <div class="overview-item">
                  <p class="label">临时状态</p>
                  <p class="value">{{ playerTempStatusText }}</p>
                </div>
                <div class="overview-item">
            ...
            <article class="panel-card hunter-panel">
              ...
              >
                ...
                <div class="nearby-main">
                  ...
                  <p class="nearby-status">当前状态：{{ person.status }}</p>
                  <p class="nearby-thought">想法：{{ person.thought }}</p>
                  <p class="line-sm">核心技能：{{ person.coreSkill || '暂无记录' }}</p>
          ...
          <section v-else class="tab-page">
            ...
            <article class="panel-card profile-block">
              ...
              <article v-if="activeProfileTab === '状态'" class="profile-card">
                <p class="line">姓名：{{ playerNameText }}</p>
                <p class="line">性别：{{ playerGenderText }}</p>
                <p class="line">堕落度：{{ playerCorruptionText }}</p>
                <p class="line">临时状态列表：{{ playerTempStatusText }}</p>
              </article>
              ...
              <article v-else-if="activeProfileTab === '特性'" class="profile-card">
                ...
                <div class="line tattoo-list-wrap">
                  ...
                  <div class="tattoo-list">
                    <p
                      v-for="(tattoo, idx) in splitDisplayLines(playerTattooListText)"
                      :key="`player-tattoo-${idx}`"
                      class="tattoo-item"
                    >
                      <span class="tattoo-glyph" aria-hidden="true">♠</span>
                      <span>{{ tattoo }}</span>
                    </p>
                  </div>
                </div>
              ...
              <article v-else class="profile-card">
                <p class="line">物品栏：{{ playerInventoryText }}</p>
              </article>
            </article>
          </section>
        </main>
    ...
    <div v-if="selectedContact" class="overlay" @click.self="selectedContact = null">
      ...
      >
        ...
        <div class="modal-block">
          ...
          <p class="line">{{ selectedContact.tempStatus }}</p>
        </div>
        <div class="modal-block">
        ...
        <div class="modal-block">
          <p class="block-title">淫纹列表（英文名 / 意象描摹）</p>
          <div class="tattoo-list">
            <p
              v-for="(tattoo, idx) in splitDisplayLines(selectedContact.tattooList)"
        ...
        <div class="modal-block">
          <p class="block-title">堕落物品列表（名称 / 效果）</p>
          <p class="line">{{ selectedContact.fallenItems }}</p>
        </div>
        <div class="modal-block">
          <p class="block-title">妆容列表（名称 / 造型）</p>
          <p class="line">{{ selectedContact.makeupList }}</p>
        </div>
        <div class="modal-block">
    ...
    <div v-if="selectedPerson" class="overlay" @click.self="selectedPerson = null">
      <section class="modal">
        ...
        <div class="modal-block">
          ...
          <p class="line">{{ selectedPerson.status }}</p>
        </div>
        <div class="modal-block">
          <p class="block-title">想法</p>
          <p class="line">{{ selectedPerson.thought }}</p>
        </div>
        <div class="modal-block">
          <p class="block-title">核心技能</p>
          <p class="line">{{ selectedPerson.coreSkill || '暂无记录' }}</p>
        </div>
...
.meta-bar {
  ...
  background:
    linear-gradient(180deg, rgba(45, 18, 58, 0.46), rgba(17, 11, 24, 0.42)),
    linear-gradient(90deg, rgba(80, 0, 115, 0.18), rgba(255, 35, 133, 0.06));
  position: relative;
  overflow: hidden;
}
...
.meta-rose {
  color: #ff7bc8;
  border-color: rgba(255, 123, 200, 0.5);
}
