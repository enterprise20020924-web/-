          ...
          <section v-else-if="activeTab === 'discover'" class="tab-page hunter-ground">
            <article class="panel-card hunter-panel">
              ...
              >
                ...
                <div class="nearby-avatar" :class="{ 'queen-avatar': isQosQueen(person.name) }">
                  <img
...
const conquerorList = computed<DiscoverItem[]>(() => {
  ...
  return Object.entries(raw as Record<string, unknown>).map(([name, value], index) => {
    ...
    const avatarUrl =
      pickImageUrl(value, ['头像框', '头像', '头像图', '头像链接', '头像URL', 'avatar', 'avatar_url']) ||
      generatedImageUrl;
    const fullImageUrl =
      pickImageUrl(value, ['全图', '立绘', '大图', '全身图', '详情图', 'fullImage', 'full_image']) || generatedImageUrl;
...
function pickImageUrl(source: unknown, keys: string[]) {
  return keys.map(key => normalizeImageUrl(get(source, key, ''))).find(Boolean) ?? '';
}
