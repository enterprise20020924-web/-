...
export type RendererHostOverlaySnapshot = {
  height: string;
  margin: string;
};
...
export function expandRendererHostForOverlay(host: HTMLIFrameElement): RendererHostOverlaySnapshot {
  const snapshot = {
    height: host.style.height,
    margin: host.style.margin,
  };
  host.style.height = RENDERER_HOST_EXPANDED_HEIGHT;
  host.style.margin = RENDERER_HOST_EXPANDED_MARGIN;
  return snapshot;
}

export function restoreRendererHostAfterOverlay(host: HTMLIFrameElement, snapshot: RendererHostOverlaySnapshot) {
  host.style.height = snapshot.height;
  host.style.margin = snapshot.margin;
}
