...
function resolveUserIdentitySpeaker(label: string, source: DialogueSource) {
  ...
  const identityAlias = identityAliases.find(alias => isSameUserIdentityName(label, alias));
  if (identityAlias !== undefined) {
    const primaryName = source.primaryUserName?.trim() ?? '';
    return primaryName.length > 0 && !isGenericUserAlias(primaryName) ? primaryName : identityAlias;
  }
  ...
  return (
    source.userAliases
      .filter(alias => !isGenericUserAlias(alias) && isSameSpeakerName(alias, label))
      .sort((left, right) => normalizeSpeakerLookupText(right).length - normalizeSpeakerLookupText(left).length)[0] ??
    null
  );
}
...
function findSpeechMapMatchForQuote(paragraphIndex: number, quoteText: string, source: DialogueSource) {
  ...
  return {
    speaker: resolveAnonymousMapSpeechSpeaker(bestMatch.entry, speaker),
    focus,
...
function normalizeSpeakerForResolvedRole(speaker: string | null, role: ReturnType<typeof resolveSegmentKind>) {
...
function splitNamedActionParagraph(
  ...
  return [
    withEmotion({
      ...
      kind: role.kind,
      side: role.side,
      speaker: normalizeSpeakerForResolvedRole(speechSpeaker, role),
      text: paragraph,
      sourceIndex: paragraphIndex,
    }),
  ];
...
function collectParagraphEvidence(paragraphs: string[], source: DialogueSource) {
  ...
  paragraphs.forEach(paragraph => {
    const carriedSpeaker = recentContextSpeakerDistance <= SPEAKER_CARRY_MAX_PARAGRAPHS ? recentContextSpeaker : null;
    const attributionSpeaker = inferAttributionParagraphSpeaker(paragraph, source);
...
function findExplicitQuoteAttributionSpeaker(segment: DialogueSegment, paragraphs: string[], source: DialogueSource) {
  ...
  const colonLabel = leadingText.trim().match(/([^：:\n]{1,36})[：:]\s*$/)?.[1] ?? null;
  const colonSpeaker = colonLabel === null ? null : resolveColonSpeaker(colonLabel, source, null);
  return leadingAttribution?.speaker ?? trailingAttribution?.speaker ?? colonSpeaker;
}
...
function resolveQuoteSegmentSpeakerByEvidence(
  ...
  const previousParagraphActor =
    addressOnlyQuote && segment.sourceIndex > 0
  ...
  if (match !== null) {
    ...
    const leadingWindow = paragraph.slice(Math.max(0, match.start - 64), match.start);
    const leadingAttribution = inferLeadingAttribution(leading, source);
    const contextualBaseSpeaker = conservativeSpeakerInference
      ? null
...
function resolveAnonymousMapSpeechSpeaker(mapEntry: DialogueMapEntry, speaker: string | null) {
...
function normalizeExactShortSpeechAnchorText(value: string) {
  return normalizeMapAnchorText(value).replace(/[，,。！？!?；;…—-]/g, '');
}

function canUseExactShortSpeechMap(segment: DialogueSegment, entry: DialogueMapEntry) {
  if (entry.kind !== 'speech' || !isDirectSpeechMapSegment(segment)) {
    return false;
  }

  const normalizedAnchor = normalizeExactShortSpeechAnchorText(entry.anchor);
  if (normalizedAnchor.length < 2 || normalizedAnchor.length >= DRIFTED_PARAGRAPH_ANCHOR_MIN_LENGTH) {
    return false;
  }

  return normalizeExactShortSpeechAnchorText(segment.text) === normalizedAnchor;
}

function compareExactShortSpeechMapPriority(
  segmentIndex: number,
  sourceParagraphNumber: number,
  left: { entry: DialogueMapEntry },
  right: { entry: DialogueMapEntry },
) {
  const leftParagraph = getDialogueMapParagraphNumber(left.entry) ?? sourceParagraphNumber;
  const rightParagraph = getDialogueMapParagraphNumber(right.entry) ?? sourceParagraphNumber;
  return (
    Math.abs(left.entry.i - (segmentIndex + 1)) - Math.abs(right.entry.i - (segmentIndex + 1)) ||
    Math.abs(leftParagraph - sourceParagraphNumber) - Math.abs(rightParagraph - sourceParagraphNumber) ||
    left.entry.i - right.entry.i
  );
}
...
function createWholeParagraphMapSpeechSegment(paragraph: string, paragraphIndex: number, source: DialogueSource) {
  ...
  const displaySpeaker = resolveAnonymousMapSpeechSpeaker(mapEntry, speaker);
...
function findAnonymousSpeechMapEntryForSourceBlock(
  segment: DialogueSegment,
  dialogueMap: DialogueMapEntry[],
  paragraphBlockIndexes: number[],
  paragraphs: string[],
  source: DialogueSource,
  ...
  const sourceBlockIndex = paragraphBlockIndexes[segment.sourceIndex];
  if (sourceBlockIndex === undefined) {
    return null;
  }

  const blockSpeechEntries = dialogueMap
    .map((entry, entryIndex) => ({ entry, entryIndex }))
    .filter(({ entry }) => entry.kind === 'speech')
    .filter(({ entry }) => {
      const paragraphNumber = getDialogueMapParagraphNumber(entry);
      return paragraphNumber !== null && paragraphBlockIndexes[paragraphNumber - 1] === sourceBlockIndex;
    });
  if (blockSpeechEntries.length === 0 || blockSpeechEntries.some(({ entry }) => entry.speaker !== null)) {
    return null;
  }

  const bestMatch = blockSpeechEntries.sort((left, right) => {
    const leftParagraph = getDialogueMapParagraphNumber(left.entry) ?? segment.sourceIndex + 1;
    const rightParagraph = getDialogueMapParagraphNumber(right.entry) ?? segment.sourceIndex + 1;
    return (
      Math.abs(leftParagraph - (segment.sourceIndex + 1)) - Math.abs(rightParagraph - (segment.sourceIndex + 1)) ||
      left.entry.i - right.entry.i
    );
  })[0];
  return {
...
function findForwardSpeechContinuationMapEntry(
  ...
  const candidates = dialogueMap
    .map((entry, entryIndex) => ({ entry, entryIndex }))
    .filter(({ entry, entryIndex }) => usedEntries.has(entryIndex) && entry.kind === 'speech' && entry.speaker !== null)
    .map(({ entry, entryIndex }) => {
      const normalizedAnchor = normalizeMapAnchorText(entry.anchor);
      ...
      const bridgeText = paragraphText
        ...
        .trim();
      if (!isTrailingSpeechAttributionDisplayText(bridgeText)) {
        return null;
      }
...
function findDialogueMapEntryForSegment(
  ...
  if (normalizedSegmentText.length > 0) {
    ...
    const exactShortSpeechAnchorMatches = dialogueMap
      .map((entry, entryIndex) => ({ entry, entryIndex }))
      .filter(match => !usedEntries.has(match.entryIndex))
      .filter(match => isParagraphDialogueMapEntry(match.entry))
      .filter(match => canUseExactShortSpeechMap(segment, match.entry))
      .sort((left, right) => compareExactShortSpeechMapPriority(segmentIndex, sourceParagraphNumber, left, right));

    if (exactShortSpeechAnchorMatches.length > 0) {
      const bestMatch = exactShortSpeechAnchorMatches[0];
      return {
        entry: bestMatch.entry,
        entryIndex: bestMatch.entryIndex,
        consumedEntryIndexes: [bestMatch.entryIndex],
    ...
    const driftedParagraphAnchorMatches = dialogueMap
      .map((entry, entryIndex) => {
        if (
          ...
          !isParagraphDialogueMapEntry(entry) ||
          !isDialogueMapEntryCompatibleWithSegment(segment, entry, paragraphText)
        ) {
          return null;
        }
    ...
    if (driftedParagraphAnchorMatches.length > 0) {
      driftedParagraphAnchorMatches.sort((left, right) =>
        compareDialogueMapCandidatePriority(left.priority, right.priority),
      );
  ...
  const anonymousBlockMatch = findAnonymousSpeechMapEntryForSourceBlock(
    segment,
    dialogueMap,
    paragraphBlockIndexes,
    paragraphs,
    source,
  );
  if (anonymousBlockMatch !== null) {
    return anonymousBlockMatch;
  }
...
function applyDialogueMapToSegments(
  ...
  return segments.map((segment, segmentIndex) => {
    ...
    const displaySpeaker = resolveAnonymousMapSpeechSpeaker(mapEntry, speaker);
    const role = resolveMappedSegmentRole(mapEntry, displaySpeaker, source);
    const mappedSpeaker = role.kind === 'narration' ? null : displaySpeaker;
...
export function splitDialogueSource(source: DialogueSource): DialogueSplitResult {
  ...
  const conservativeSpeakerInference = shouldUseConservativeSpeakerInference(source);
  const paragraphRecords = content
    .split(/\n{2,}/)
    .flatMap((block, blockIndex) => block.split('\n').map(text => ({ text: text.trim(), blockIndex })))
    .filter(record => record.text.length > 0);
  const paragraphs = paragraphRecords.map(record => record.text);
  const paragraphBlockIndexes = paragraphRecords.map(record => record.blockIndex);
  ...
  const mappedSegments = applyDialogueMapToSegments(segments, effectiveSource, paragraphs, paragraphBlockIndexes);
  const resolvedSegments = resolveSegmentsWithEvidence(mappedSegments, paragraphs, effectiveSource);
  const attributionCleanedSegments = removeTrailingAttributionDisplaySegments(resolvedSegments, effectiveSource);
