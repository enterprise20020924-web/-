...
const FOCUS_MENTION_AFTER_PATTERN =
  /^\s*(?:，|,)?\s*(?:的[^。！？!?；;\n]{0,24})?(?:正|正在|仍然|仍旧|依旧|始终|一直|只是|低头|抬手|抬头|抬起头|转头|转过头|转身|回头|看向|垂眸|坐|站|靠|走|走来|走过来|走进|走进来|跑|停|拿|拿着|捧|捧着|抱|抱着|夹|夹着|翻|把|向|朝|对|在|坐在|站在|靠在|位于|端|端着|喝|喝了|写|写下|合上|停|停了|落|落回|移|移开|清点|核对|打勾|打着勾)/;
...
function inferVoiceAttributionParagraphSpeaker(paragraph: string, source: DialogueSource) {
  const matches = createSpeakerCandidates(source)
    ...
    .filter(match => VOICE_ATTRIBUTION_AFTER_SPEAKER_PATTERN.test(match.after))
    .sort(
      (left, right) =>
