...
export function buildNarrativeChatRows(
  ...
  const rows = segments.flatMap<NarrativeChatRow>(segment => {
    if (segment.kind === 'narration' || isNarrativeChatStructuralNarration(segment)) {
      return createNarrativeChatNarrationRows(segment);
    }
...
function createNarrationRow(
  segment: DialogueSegment,
  text: string,
  partIndex: number,
  sourceIndex = segment.sourceIndex,
): NarrativeChatNarrationRow {
  return {
...
function createNarrativeChatNarrationRows(segment: DialogueSegment) {
  const sourceParts = segment.sourceParagraphParts ?? [
    {
      text: segment.text,
      sourceIndex: segment.sourceIndex,
    },
  ];
  return sourceParts.flatMap<NarrativeChatNarrationRow>((sourcePart, partIndex) => {
    const narrationText = stripNarrationAttribution(sourcePart.text, segment);
    return narrationText.length > 0
      ? [createNarrationRow(segment, narrationText, partIndex, sourcePart.sourceIndex)]
      : [];
  });
}
