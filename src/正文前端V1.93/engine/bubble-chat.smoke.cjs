...
{
  const galSegments = splitDialogueSource({
    messageId: 1,
    content: ['第一段旁白。', '第二段旁白。', '第三段旁白。'].join('\n\n'),
    knownCharacters: [],
    userAliases: ['{{user}}'],
  }).segments;
  assert.equal(galSegments.length, 1, 'GAL mode should retain its compact narration page');
  assert.match(galSegments[0].id, /-merged-3$/, 'GAL narration page should record its merged source count');

  const chatRows = buildNarrativeChatRows(galSegments, {
    resolveSpeakerName: () => '',
    resolveAffiliation: () => null,
    resolveAvatarCandidates: () => [],
  });
  assert.deepEqual(
    chatRows.map(row => row.text),
    ['第一段旁白。', '第二段旁白。', '第三段旁白。'],
    'chat mode must restore each natural narration paragraph as an independent row',
  );
}

{
  const galSegments = splitDialogueSource({
    messageId: 1,
    content: '同一自然段的第一句。紧接着是第二句。最后是第三句。',
    knownCharacters: [],
    userAliases: ['{{user}}'],
  }).segments;
  assert.equal(galSegments.length, 1, 'one natural paragraph may still be compacted into one GAL page');
  ...
  assert.deepEqual(
    chatRows.map(row => row.text),
    ['同一自然段的第一句。紧接着是第二句。最后是第三句。'],
    'chat mode must not split sentences that belong to the same natural paragraph',
  );
}
...
assert.equal(resolveFullbodyPortraitProfile('普通新生'), null, 'anonymous focus must not invent a standee asset');
