...
const activeNpcLabel = computed(() => {
  ...
  return knownCharacters.value[0] ?? null;
});
...
const portraitNpcLabel = computed(() => {
  ...
  if (isStreaming.value) {
    return findPreviousNpcSpeaker(segments.value, currentIndex.value) ?? knownCharacters.value[0] ?? null;
  }
...
const activeSpeakerAffiliation = computed(() => {
  ...
  return activeNpcLabel.value === null ? null : resolveCharacterProfile(activeNpcLabel.value, currentSegment.value.text)?.affiliation ?? null;
});
...
const npcPortraitLayers = computed(() => {
  const layers = new Map<string, PortraitLayer>();
  const defaultNpcLabel = knownCharacters.value[0] ?? null;
...
function resolveNpcNeutralPortraitUrl(speakerName: string, contextText = '') {
  return (
    ...
    resolveMoodPortrait([speakerName], 'neutral') ??
...
function resolveNpcPortraitUrl(speakerName: string, mood: DialogueMood, contextText = '') {
  ...
  return (
    resolveMoodPortrait([speakerName], mood) ??
