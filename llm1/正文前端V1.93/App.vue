<template>
  <div class="bp-desktop-scale-shell">
    <div class="bp-desktop-scale-surface">
      <article
        :class="[
          'ContentChatRenderer',
          {
      ...
      >
        ...
        >
          ...
          />
          <Teleport :to="minimizedOverlayPortalTarget ?? 'body'" :disabled="!functionPageOpenedFromMinimized">
            <div
              :class="[
                'bp-desktop-overlay-host',
                {
                  ContentChatRenderer: functionPageOpenedFromMinimized,
                  'bp-minimized-function-portal': functionPageOpenedFromMinimized,
                },
              ]"
              :style="functionPageOpenedFromMinimized ? desktopAssetCssVariables : undefined"
            >
              <DesktopOverlays
...
const minimizedOverlayPortalTarget = ref<HTMLElement | null>(null);
...
const dialogueSource = computed<DialogueSource>(() => ({
  ...
  primaryUserName: userStatus.value.alias,
  secondaryUserNames: [],
...
onBeforeUnmount(() => {
  minimizedOverlayPortalTarget.value = null;
...
function openManualBattlePage(event: MouseEvent) {
  minimizedOverlayPortalTarget.value = resolveParentDocumentBody(event);
...
function restoreMinimizedAfterFunctionPage() {
  ...
  const returnFocus = functionPageReturnFocus.value;
  functionPageOpenedFromMinimized.value = false;
  minimizedOverlayPortalTarget.value = null;
  isNarrativeMinimized.value = true;
  void nextTick(() => {
...
function resolveParentDocumentBody(event: Event) {
  ...
  try {
    ...
    return frameElement?.ownerDocument.body ?? ownerDocument.body;
  } catch {
