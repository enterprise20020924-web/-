...
export function removeDialogueMapOutsideCot(message: string) {
  ...
  const inlineDialogueMarkerPattern = /<台词\d+\b[^<>]*?\/?>/giu;
  let inlineMarkerMatch = inlineDialogueMarkerPattern.exec(message);
  while (inlineMarkerMatch !== null) {
    if (!isOffsetInsideRanges(inlineMarkerMatch.index, cotRanges)) {
      removalRanges.push({
        start: inlineMarkerMatch.index,
        end: inlineMarkerMatch.index + inlineMarkerMatch[0].length,
      });
    }
    inlineMarkerMatch = inlineDialogueMarkerPattern.exec(message);
  }
...
const GENERATED_PARAGRAPH_LABEL_PATTERN = /(^|\n(?:[ \t]*\n)+)[ \t]*P\d{1,4}\s*[:：][ \t]*(?:\r?\n)?/gim;

export function stripGeneratedParagraphLabels(text: string) {
  const matches = [...text.matchAll(GENERATED_PARAGRAPH_LABEL_PATTERN)];
  if (matches.length < 2) {
    return text;
  }

  GENERATED_PARAGRAPH_LABEL_PATTERN.lastIndex = 0;
  return text.replace(GENERATED_PARAGRAPH_LABEL_PATTERN, '$1');
}

export function getMessageDialogueMapSourceText(message: string) {
  const cleanedMessage = stripDialogueMapBlocks(message);
  return stripGeneratedParagraphLabels(stripHiddenBlocks(extractContent(cleanedMessage) ?? cleanedMessage));
}
...
function parseChoiceOptions(content: string): ChoiceOption[] {
  const cleanedContent = cleanChoiceOptionText(
    content
      ...
      .replace(/<w2p_analysis\b[^>]*>[\s\S]*?<\/w2p_analysis>/gi, ''),
  );
...
export function extractChoiceOptions(message: string): ChoiceOption[] {
  ...
  const w2pBlocks = Array.from(source.matchAll(/<w2p\b[^>]*>([\s\S]*?)<\/w2p>/gi));
  const preferredBlocks = choiceBlocks.length > 0 ? choiceBlocks : optionBlocks.length > 0 ? optionBlocks : w2pBlocks;
